...
不相同。 CAU 对于群集应用程序类型是不可知的。 CAU 是\-更新在群集 Api 和 PowerShell cmdlet 之上分层的解决方案的外部群集。 同样，CAU 可以协调 Windows Server 故障转移群集中配置的任何群集应用程序的更新。  
...
不相同。 CAU 是\-作用域更新工具的群集，因此它只允许你选择要更新的群集。 如果希望更新单独一个节点，你可使用 CAU 以外的已有服务器工具。  
