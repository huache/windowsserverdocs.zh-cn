...
CAU 安装 \(**microsoft.windowsupdateplugin**和**microsoft.hotfixplugin**\) 的插件\-提供可选择的其他选项。 在 CAU UI 中，在你为中的\-插件配置更新运行选项之后，这些选项将显示在 "**其他选项**" 页上。 如果你使用的是 CAU PowerShell cmdlet，则在参数中将这些选项配置为可选的插件\-。 有关详情，请参阅本主题后面的 [使用 Microsoft.WindowsUpdatePlugin](#BKMK_WUP) 和 [使用 Microsoft.HotfixPlugin](#BKMK_HFP) 。  
