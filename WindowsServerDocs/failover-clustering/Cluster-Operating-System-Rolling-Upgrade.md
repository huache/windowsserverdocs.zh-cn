...
- 使用以下方法之一验证 Hyper-v 节点具有支持二级寻址表（SLAT）的 Cpu。      -请查看[是否与 SLAT 兼容？WP8 SDK Tip 01](https://blogs.msdn.com/b/devfish/archive/2012/11/06/are-you-slat-compatible-wp8-sdk-tip-01.aspx)文章，其中介绍了两种方法来检查 cpu 是否支持[SLATs，以](https://technet.microsoft.com/sysinternals/cc835722)确定 CPU 是否支持 SLAT。
 ...
 **图4：中间状态：所有节点已升级到 windows server 2016，可用于更新-update-clusterfunctionallevel （第3阶段）**
...
1. 为操作系统升级准备群集，如下所示：
    ...
    4. 使用 cmdlet 检查所有群集节点是否处于联机状态/running/up [`Get-ClusterNode`](https://docs.microsoft.com/powershell/module/failoverclusters/Get-ClusterNode?view=win10-ps) （参见图7）。
    ...
    5. 如果正在运行群集感知更新（CAU），请使用**群集感知更新**UI 或 cmdlet 验证是否当前正在运行 cau [`Get-CauRun`](https://docs.microsoft.com/powershell/module/clusterawareupdating/Get-CauRun?view=win10-ps) （参见图8）。 使用 cmdlet 停止 CAU [`Disable-CauClusterRole`](https://docs.microsoft.com/powershell/module/clusterawareupdating/Disable-CauClusterRole?view=win10-ps) （请参阅图9），以防止在群集 OS 滚动升级过程中任何节点被 CAU 暂停和排出。

        ![截图显示 Invoke-caurun cmdlet 的输出 ](media/Cluster-Operating-System-Rolling-Upgrade/Cluster_RollingUpgrade_GetCAU.png) **图8：使用 [`Get-CauRun`](https://docs.microsoft.com/powershell/module/clusterawareupdating/Get-CauRun?view=win10-ps) Cmdlet 确定群集感知更新是否正在群集上运行**

        ![截图显示 Add-cauclusterrole cmdlet 的输出 ](media/Cluster-Operating-System-Rolling-Upgrade/Cluster_RollingUpgrade_DisableCAU.png) **图9：使用 [`Disable-CauClusterRole`](https://docs.microsoft.com/powershell/module/clusterawareupdating/Disable-CauClusterRole?view=win10-ps) Cmdlet 禁用群集感知更新角色**
...
2. 对于群集中的每个节点，完成以下操作：
    1. 使用群集管理器 UI，选择节点并使用 "**暂停" |排出节点**以排出节点（请参阅图10）或使用 [`Suspend-ClusterNode`](https://docs.microsoft.com/powershell/module/failoverclusters/Suspend-ClusterNode?view=win10-ps) cmdlet （参见图11）。
        ...
        ![截图显示 Start-clusternode cmdlet 的输出 ](media/Cluster-Operating-System-Rolling-Upgrade/Cluster_RollingUpgrade_SuspendNode.png) **图11：使用 [`Suspend-ClusterNode`](https://docs.microsoft.com/powershell/module/failoverclusters/Suspend-ClusterNode?view=win10-ps) Cmdlet 从节点排出角色**
    ...
    3.  在 setup.exe 中使用**自定义：仅安装 windows （高级）** 安装（参见图13）选项来重新格式化系统驱动器，并在节点上执行 Windows Server 2016 的 "干净操作系统安装"。 避免选择 "**升级：安装 Windows 并保留文件、设置和应用程序**" 选项，因为群集 OS 滚动升级不鼓励就地升级。
    ...
    13. 使用故障转移群集管理器 UI 或 [`Add-ClusterNode`](https://docs.microsoft.com/powershell/module/failoverclusters/Add-ClusterNode?view=win10-ps) cmdlet （参见图16）将节点添加到群集。
         ...
         **图16：使用 [`Add-ClusterNode`](https://docs.microsoft.com/powershell/module/failoverclusters/Add-ClusterNode?view=win10-ps) Cmdlet 向群集添加节点**
    ...
    14. 将 Windows Server 2016 节点成功添加到群集后，可以（可选）将某些群集工作负荷移动到新添加的节点，以便在群集中重新均衡工作负荷，如下所示：
        ...
        1. 使用**Live Migration**虚拟机或 cmdlet 的故障转移群集管理器中的实时迁移 [`Move-ClusterVirtualMachineRole`](https://docs.microsoft.com/powershell/module/failoverclusters/Move-ClusterVirtualMachineRole?view=win10-ps) （请参阅图17）来执行虚拟机的实时迁移。

            ```PowerShell
            Move-ClusterVirtualMachineRole -Name VM1 -Node robhind-host3
            ```

        2. 对于**Move**其他群集工作负载，请使用从故障转移群集管理器或 [`Move-ClusterGroup`](https://docs.microsoft.com/powershell/module/failoverclusters/Move-ClusterGroup?view=win10-ps) cmdlet 进行移动。
...
3. 如果每个节点都已升级到 Windows Server 2016 并添加回群集，或者已逐出任何其他 Windows Server 2012 R2 节点，请执行以下操作：
    ...
    > -   在 [`Update-ClusterFunctionalLevel`](https://docs.microsoft.com/powershell/module/failoverclusters/Update-ClusterFunctionalLevel?view=win10-ps) 运行 cmdlet 之前，该过程是完全可逆的，可以将 Windows server 2012 R2 节点添加到此群集，并且可以删除 Windows server 2016 节点。
    > -   [`Update-ClusterFunctionalLevel`](https://docs.microsoft.com/powershell/module/failoverclusters/Update-ClusterFunctionalLevel?view=win10-ps)运行 cmdlet 后，新功能将可用。
    ...
    2.  使用 cmdlet 检查所有群集节点是否均已联机且正在运行 [`Get-ClusterNode`](https://docs.microsoft.com/powershell/module/failoverclusters/Get-ClusterNode?view=win10-ps) 。
    ...
    4.  [`Update-ClusterFunctionalLevel`](https://docs.microsoft.com/powershell/module/failoverclusters/Update-ClusterFunctionalLevel?view=win10-ps)运行 cmdlet 后，可以使用新功能。
...
4. Windows Server 2016-恢复常规群集更新和备份：
    ...
    1. 如果以前运行过 CAU，请使用 CAU UI 重启它或使用 [`Enable-CauClusterRole`](https://docs.microsoft.com/powershell/module/clusterawareupdating/Enable-CauClusterRole?view=win10-ps) cmdlet （请参阅图20）。
        ...
        ![截图显示 Add-cauclusterrole ](media/Cluster-Operating-System-Rolling-Upgrade/Cluster_RollingUpgrade_EnableCAUClusterRole.png) **图20：使用 [`Enable-CauClusterRole`](https://docs.microsoft.com/powershell/module/clusterawareupdating/Enable-CauClusterRole?view=win10-ps) Cmdlet 启用群集感知更新角色**的输出
...
5. 启用并使用 Hyper-v 虚拟机上的 Windows Server 2016 功能。
   ...
   3. 在群集中的每个 Hyper-v 主机节点上，可以通过使用用户计划简短的维护时段、备份、关闭虚拟机并运行 cmdlet 来升级 Hyper-v VM 配置版本 [`Update-VMVersion`](https://docs.microsoft.com/powershell/module/hyper-v/Update-VMVersion?view=win10-ps) （请参阅图22）。 这将更新虚拟机版本并启用新的 Hyper-v 功能，从而无需以后再进行 Hyper-v 集成组件（IC）更新。 此 cmdlet 可以从托管 VM 的 Hyper-v 节点运行，也 `-ComputerName` 可以使用参数远程更新 Vm 版本。 在此示例中，我们将 VM1 的配置版本从5.0 升级到7.0，以利用与此 VM 配置版本关联的许多新 Hyper-v 功能，如生产检查点（应用程序一致性备份）和二进制 VM 配置文件。
...
**[`Update-ClusterFunctionalLevel`](https://docs.microsoft.com/powershell/module/failoverclusters/Update-ClusterFunctionalLevel?view=win10-ps)节点关闭或暂停时，是否可以运行 cmdlet？**
否。 所有群集节点都必须处于启用状态，并且在 active 成员身份中，才能使该 [`Update-ClusterFunctionalLevel`](https://docs.microsoft.com/powershell/module/failoverclusters/Update-ClusterFunctionalLevel?view=win10-ps) cmdlet 正常运行。
...
**如果在成功运行后发现群集中的问题，该怎么办 [`Update-ClusterFunctionalLevel`](https://docs.microsoft.com/powershell/module/failoverclusters/Update-ClusterFunctionalLevel?view=win10-ps) ？**
如果在运行之前已使用系统状态备份对群集数据库进行了备份 [`Update-ClusterFunctionalLevel`](https://docs.microsoft.com/powershell/module/failoverclusters/Update-ClusterFunctionalLevel?view=win10-ps) ，则应该能够在 Windows Server 2012 R2 群集节点上执行权威还原，并还原原始群集数据库和配置。
