...
- **群集集**
    ...
    使用群集集，可以将单个软件定义的数据中心中的服务器数增加 (SDDC) 解决方案，超出群集的当前限制。 这是通过将多个群集分组到一个群集集中来实现的，这是多个故障转移群集的松耦合分组：计算、存储和超聚合。
    利用群集集，可以将联机虚拟机 (在群集集中的群集之间进行实时迁移) 。
...
- **文件共享见证增强**
  ...
  - 磁盘见证缺少共享驱动器。 这可能是存储空间直通超聚合配置、SQL Server Always On 可用性组 (AG) 或 * Exchange Database 可用性组 (DAG) ，它们都不使用共享磁盘。
...
-   为存储帐户提供了很低的成本， (每个 blob 文件写入非常小的数据，仅当群集节点的状态) 发生更改时，blob 文件才更新一次。
...
- 群集日志文件的几项增强功能 (例如时区信息和 DiagnosticVerbose 日志) ，从而更容易排查故障转移群集问题。 有关详细信息，请参阅[Windows Server 2016 故障转移群集故障排除增强-群集日志](https://techcommunity.microsoft.com/t5/failover-clustering/windows-server-2016-failover-cluster-troubleshooting/ba-p/372005)。
...
虚拟机负载平衡是故障转移群集中的一项新功能，用于简化群集中各个节点上的虚拟机的无缝负载平衡。 基于节点上的虚拟机内存和 CPU 使用率标识过度提交的节点。 然后，会将虚拟机从已过度提交的节点移动到具有可用带宽 (的节点（如果适用) ）， (实时迁移) 。 可以优化平衡的做法，以确保群集的最佳性能和利用率。 默认情况下，在 Windows Server 2016 Technical Preview 中启用负载平衡。 但是，启用 SCVMM 动态优化后，将禁用负载平衡。

### <a name="virtual-machine-start-order"></a><a name="BKMK_VMStartOrder"></a>虚拟机启动顺序

虚拟机启动顺序是故障转移群集中的一项新功能，它为虚拟机 (和所有组) 在群集中引入了开始订单业务流程。 现在可将虚拟机分组为层，并且可以在不同的层之间创建启动顺序依赖关系。 这可确保先启动 (的最重要的虚拟机，例如域控制器或实用工具虚拟机) 。 虚拟机在依赖的虚拟机也已启动之前，不会启动。
