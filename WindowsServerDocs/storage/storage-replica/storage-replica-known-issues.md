...
在 Windows Server 2016 中，配置复制时，源服务器和目标服务器可能在初始同步期间显示多个 **StorageReplica\Admin*事件日志4004警告，状态为 "不能完成 API 的系统资源不足"。 你也很可能会看到 5014 错误。 这些错误表示服务器没有足够的可用内存 (RAM) 来执行初始同步和运行工作负荷。 从功能和应用程序（而不是从存储副本）添加 RAM 或减少使用的 RAM。
...
当你使用 **fltmc.exe attach svhdxflt*-跳过运行 CSV 的 hyper-v 主机的要求时，会发生此行为。 不支持使用此命令，且仅用于测试和演示目的。
...
使用 `New-Partition**` cmdlet 创建卷并将其格式化来替代 `New-Volume`，因为后一个 cmdlet 可能会在不同的存储阵列上轮循卷的大小。 如果已创建了 NTFS 卷，则可使用 `Resize-Partition` 以增大或收缩其中的一个卷以匹配另一个卷（ReFS 卷不能执行此操作）。 如果使用 **diskmgmt.msc*或**服务器管理器**，则不会进行舍入。
...
这是由于选择与系统驱动器位于同一分区中的数据卷 (例如，使用 Windows 文件夹) 的 **C：*-驱动器。 例如，在包含从同一分区创建的 **C：*-和 **D：* 卷的驱动器上。 在存储副本中不支持此功能；你必须选择不同的卷用于复制。
...
此问题已在 Windows 10 的累积更新中解决，版本 1607 (周年更新) 和 Windows Server 2016： 2016 (KB3201845) 。
...
在增大源数据分区之前，请确保目标数据分区具有足够的空间，以使其大小相等。 阻止了存储副本保护的数据分区的收缩。
...
- 异步目标无法与源保持同步，以使最新的目标日志记录不再出现在源日志中。 目标将开始阻止复制。 块复制完成后，RPO 应变为可用。
...
请注意 `Status: "{Access Denied}"` ， `A process has requested access to an object, but has not been granted those access rights.` 这是存储副本中的已知问题，在2017年9月12日的质量更新中已修复-KB4038782 (OS Build 14393.1715) https://support.microsoft.com/help/4038782/windows-10-update-kb4038782
...
如果 `ClusterFunctionalLevel = 9` 为，则为实现此节点上的存储副本所需的 Windows 2016 update-clusterfunctionallevel 版本。
如果 ClusterFunctionalLevel 是不 9，则需要更新 ClusterFunctionalLevel 才能在此节点上实现存储副本。

若要解决此问题，请运行 PowerShell cmdlet： [update-clusterfunctionallevel](/powershell/module/failoverclusters/update-clusterfunctionallevel)，提高群集功能级别。
...
通过备份、VSSADMIN 等创建 VSS 快照 () 存储副本节点挂起时，你必须强制重新启动该节点才能进行恢复。 没有错误，只是服务器的硬挂起。
...
发生此问题的原因在于，与较慢的媒体相比，存储副本的日志机制内的体系结构限制与 NVME 的延迟非常低。 当使用存储空间直通缓存时，所有存储副本日志的 i/o 以及所有最近的读取/写入 IO 应用程序将出现在缓存中，而从不会出现在性能层或容量层上。 这意味着所有存储副本活动都在同一速度介质上发生-支持此配置，但不建议 (参阅 https://aka.ms/srfaq 日志建议) 。
...
如果在两个驱动器上同时启用了 BitLocker (日志驱动器和数据驱动器) 并且在这两个存储副本驱动器中，如果主服务器重新启动，则即使从 BitLocker 解锁日志驱动器后，也无法访问主驱动器。
