...
新目录的名称基于文件管理任务的名称以及运行任务的时间。 找到过期文件后，会将其移动到新目录中，同时保留其原始目录结构。
...
5. 另外，在**通知**选项卡上单击**添加**以发送电子邮件通知、记录事件、运行命令或编写任务对文件执行操作之前指定最少天数的脚本。
   ...
   - 若要配置电子邮件通知，请单击**电子邮件**选项卡，然后输入以下信息：
     ...
     - 若要配置邮件，请编辑所提供的默认主题行和邮件正文。 方括号中的文本会插入关于导致通知的配额事件的变量信息。 例如，\[的**源文件所有者\]** 变量将插入其文件即将过期的用户的名称。 若要在文本中插入其他变量，请单击**插入变量**。
