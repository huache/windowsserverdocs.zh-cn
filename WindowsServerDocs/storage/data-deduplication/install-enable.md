...
若要安装重复数据删除，请以管理员身份运行以下 PowerShell 命令：`Install-WindowsFeature -Name FS-Data-Deduplication`
...
1. **我的工作负荷数据集是否具有足够的重复数据可从启用重复数据删除中受益？**
    ...
    &nbsp;运行 DDPEval.exe 将返回类似于下面的输出： &nbsp;`Data Deduplication Savings Evaluation Tool`
