...
1. 在所有源服务器以及运行 Windows Server 2012 R2 或 Windows Server 2016 的任何目标服务器上，在 Windows 管理中心中连接到每台服务器，中转到**服务器管理器**（在 Windows 管理中心中） > **Firewall** > **传入规则**，然后检查是否已启用以下规则：
    - 文件和打印机共享 (SMB-In)
    - Netlogon 服务（NP-IN）
...
1. 选择 "**新建作业**"，为作业命名，然后选择是否迁移使用 Samba 的 Windows 服务器和群集或 Linux 服务器。 然后，选择“确定”。
...
4. 选择 "**开始扫描**"。<br>在扫描完成后，页面将更新以显示。
    ![屏幕截图，显示可以扫描的服务器](media/migrate/inventory.png)**图2：清点服务器**
...
2. 在 "切换**到新服务器**" > **输入凭据**"页上，选择"**下一步**"以使用之前键入的凭据。
...
4. 指定在转换后，要用于源服务器的 IP 地址将其地址移至目标。 你可以使用 DHCP 或静态地址。 如果使用静态地址，则新的子网必须与旧子网相同，否则转换将失败。
    ![屏幕截图，其中显示了源服务器及其 IP 地址和计算机名称，以及在切换后它们将被替换为](media/migrate/cutover.png)**图4：源服务器以及其网络配置将如何移动到目标**
