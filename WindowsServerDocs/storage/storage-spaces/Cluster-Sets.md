...
8. 若要验证群集集创建进程是否创建了一个 SMB 共享（标识为 Volume1，或者在每个群集成员的 CSV 卷的基础结构 SOFS 上，ScopeName 为基础结构文件服务器名称和路径），请执行以下操作：
...
**问题：** 在灾难恢复情况下，是否可以跨容错域对虚拟机进行故障转移（假设整个容错域已关闭）？ <br>
