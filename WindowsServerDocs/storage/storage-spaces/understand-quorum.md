...
- **群集仲裁**：此操作在群集级别运行 (即，你可以丢失节点并使群集保持) 
- **池仲裁**：当启用存储空间直通时，此操作在池级别运行 (即，你可以丢失节点和驱动器，并使池保持) 。 存储池设计用于群集和非群集方案，这就是它们具有不同仲裁机制的原因。
...
但*大多数*情况下，如果群集中的节点总数为奇数 (例如，在五个节点的群集中的三个节点) 。 那么，具有偶数个节点的群集怎么办 (说，四个节点群集) ？

群集可通过两种方式将*投票总数*设置为奇数：

1. 首先，通过添加具有额外投票的*见证*服务器来执行*此项。* 这需要用户设置。
2.    或者，它可以*向下移动*一个不幸节点的投票 (根据需要自动进行) 。
...
动态仲裁允许动态地将投票分配给节点，以避免丢失大多数投票，并使群集能够使用一个节点 (称为最后一站) 。 让我们以一个四节点的群集为例。 假定仲裁需要3个投票。
...
我们刚刚讨论过群集仲裁，它在群集级别运行。 现在，让我们深入了解池仲裁，它在池级别上运行 (即，可以丢失节点和驱动器，并使池保持) 。 存储池设计用于群集和非群集方案，这就是它们具有不同仲裁机制的原因。
...
当驱动器出现故障时，或者当某个部分驱动器失去与另一个子集的联系时，仍然存在的驱动器需要验证它们是否构成了*大部分*的池以保持联机状态。 如果他们无法验证，它们将处于脱机状态。 池是离线的实体，或基于它是否有足够的磁盘用于仲裁 (50% + 1) 的实体。 池资源所有者 (活动群集节点) 可以为 + 1。

但池仲裁在以下方面与群集仲裁的工作方式不同：

- 池使用群集中的一个节点作为见证服务器，作为群集资源) 所有者 (此节点中的一半驱动器。
- 池没有动态仲裁
- 池未实现其自己的删除投票的版本
...
每个16个驱动器都有一个投票，两个节点还有一个投票 (，因为它是池资源所有者) 。 *大多数*都是由**16 个投票**组成的。 如果节点三和四出现故障，则仍存在的子集具有8个驱动器和池资源所有者，即9/16 投票。 因此，池置。
...
每个16个驱动器都有一个投票，并且节点2也有一个投票 (，因为它是池资源所有者) 。 *大多数*都是由**16 个投票**组成的。 首先，驱动器7出现故障。 如果节点三和四出现故障，则仍存在的子集具有7个驱动器和池资源所有者，即8/16 投票。 因此，该池并不会出现任何问题。
...
24个驱动器中的每一个都有一个投票，两个节点还有一个投票 (，因为它是池资源所有者) 。 *大多数*都由**24 个投票**决定。 如果节点三和四出现故障，则仍存在的子集具有8个驱动器和池资源所有者，即9/24 投票。 因此，该池并不会出现任何问题。

![池仲裁3](media/understand-quorum/pool-3.png)

- 可经受一个服务器故障：**是**。
- 在出现一次服务器故障后，另一种： * * 依赖于 * * (如果两个节点三到四个均关闭，但可以经受所有其他方案。
- 一次可能会经受两个服务器故障： * * 依赖于 * * 如果两个节点都出现故障，则 (不会保留下来，但可以经受所有其他方案。

### <a name="pool-quorum-recommendations"></a>池仲裁建议

- 确保群集中的每个节点都是对称的 (每个节点具有相同数量的驱动器) 
