...
>[!NOTE]
>默认输出位置将是 <working_directory> 的 \NCDiagnostics\ 目录。 可以使用参数更改默认的输出目录 `-OutputDirectory` 。

可在此目录中的文件_diagnostics-slbstateResults.Js上_找到 SLB 配置状态信息。
...
1. 组织检查将阻止流量的虚拟子网或 VM 网络接口上是否没有指定的分布式防火墙策略。
...
1. 宿主在 ``Get-ProviderAddress`` 托管两个有问题的租户虚拟机的 hyper-v 主机上运行，然后 ``Test-LogicalNetworkConnection`` ``ping -c <compartment>`` 从 hyper-v 主机运行或，验证 HNV 提供程序逻辑网络上的连接
2.  宿主请确保 hyper-v 主机和 Hyper-v 主机之间的任何第2层交换设备上的 MTU 设置正确。 ``Test-EncapOverheadValue``在所有有问题的 hyper-v 主机上运行。 还要检查两者之间的所有第2层交换机是否设置为至少1674字节，以最大限度地降低160字节的系统开销。
3.  宿主如果 PA IP 地址不存在并且/或 CA 连接中断，请检查以确保已收到网络策略。 运行 ``Get-PACAMapping`` 以查看创建覆盖虚拟网络所需的封装规则和 CA-PA 映射是否已正确建立。
...
你可以随时使用 cmdlet 更改日志记录设置 ``Set-NetworkControllerDiagnostic`` 。 可以更改以下设置：
