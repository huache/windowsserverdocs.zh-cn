---
title: SDN 的内部 DNS 服务 (iDS)
description: 本主题说明如何使用与 Windows Server 2016 中的软件定义的网络集成的内部 DNS (Idn) 为托管的租户工作负荷提供 DNS 服务。
manager: grcusanz
...
如果你使用的是云服务提供者 \( CSP \) 或计划在 Windows Server 2016 中部署软件定义的网络 SDN 的企业 \( \) ，则可以使用 \( \) 与 SDN 集成的内部 DNS idn 向托管的租户工作负荷提供 DNS 服务。

托管虚拟机 \( vm \) 和应用程序需要 DNS 在其自己的网络中以及 Internet 上的外部资源进行通信。 通过 Idn，你可以为租户提供 DNS 名称解析服务，用于隔离本地命名空间和 Internet 资源。
...
- 从租户 VM 解析 Internet 名称的递归 DNS 服务。
...
租户 VM 完全限定的域名 \( fqdn \) 包含计算机名称和虚拟网络的 DNS 后缀字符串（采用 GUID 格式）。 例如，如果你有一个名为 TENANT1 的租户 VM，该 VM 位于虚拟网络 contoso，本地，则该 VM 的 FQDN 为 TENANT1。*vn*，其中*Vn*是用于虚拟网络的 DNS 后缀字符串。
