---
title: RAS 网关部署体系结构
description: 你可以使用本主题来了解云服务提供商 (CSP) 在 Windows Server 2016 中部署 RAS 网关，包括 RAS 网关池、路由发送程序，以及为各个租户部署多个网关。
manager: grcusanz
...
ms.locfileid: "87952288"
---
# <a name="ras-gateway-deployment-architecture"></a>RAS 网关部署体系结构

>适用于：Windows Server（半年频道）、Windows Server 2016

你可以使用本主题来了解云服务提供商 (CSP) 部署 RAS 网关，包括 RAS 网关池、路由发送程序，以及为各个租户部署多个网关。
...
-   [添加新租户和客户地址 (CA) Space EBGP 对等互连](#bkmk_tenant)
...
边界网关协议 (BGP) 路由反射器功能现在包含在 RAS 网关中，并提供了一种用于 BGP full 网格拓扑的替代方法，该拓扑通常是路由器之间的路由同步所必需的。 对于完全网格同步，所有 BGP 路由器必须与路由拓扑中的所有其他路由器连接。 然而，使用路由反射器时，路由反射器是唯一连接到其他所有路由器的路由器（称为 BGP 路由反射器客户端），因此可以简化路由同步并减少网络流量。 路由反射器学习所有路由，计算最佳路由，并将最佳路由重新分发到其 BGP 客户端。
...
这两个网关（GW2 和 GW3）是网络控制器在 CSP 将 Contoso 和 Woodgrove 租户添加到其基础结构时配置的第一个 RAS 网关。 因此，这两个网关配置为 (或租户) 的相应客户的路由发送程序。 GW2 是 Contoso 路由反射器，GW3 是 Woodgrove 路由反射器，除了是与 Contoso 洛杉矶总部站点的 VPN 连接的 CSP RAS 网关终止点。
...
网络控制器将 Hyper-v 网络虚拟化策略推送到 Contoso 和 Woodgrove 虚拟网络，并将 RAS 策略推送到 RAS 网关，并将负载平衡策略推送到配置为软件负载平衡池的 Multiplexers ( (Mux) 。

## <a name="adding-new-tenants-and-customer-address-ca-space-ebgp-peering"></a><a name="bkmk_tenant"></a>添加新租户和客户地址 (CA) Space eBGP 对等互连
...
4.  根据是否将 CA 空间路由配置为使用静态配置的网络或动态 BGP 路由，网络控制器将在 RAS 网关 VM 和路由反射器中配置相应的静态路由、BGP 邻居或两者。
    ...
    > -   由于 RAS 网关池位于软件负载平衡器之后 (SLBs) ，因此租户的站点到站点 VPN 地址每个使用单个公共 IP 地址，称为虚拟 IP 地址 (VIP) ，SLBs 将其转换为数据中心内部 IP 地址，该地址称为动态 IP 地址 (DIP) ，用于路由企业租户的流量。 通过 SLB 进行的公共到专用 IP 地址映射可确保在企业站点和 CSP RAS 网关和路由发送程序之间正确建立站点到站点 VPN 隧道。
    >
    >     有关 SLB、Vip 和 Dip 的详细信息，请参阅用于[SDN 的软件负载平衡 &#40;SLB&#41;](../../../sdn/technologies/network-function-virtualization/Software-Load-Balancing--SLB--for-SDN.md)。

5.  为新租户建立企业站点与 CSP 数据中心 RAS 网关之间的站点到站点 VPN 隧道后，与隧道关联的静态路由将在隧道的企业和 CSP 端自动进行配置。
...
同样，在设置了 Hyper-v 网络虚拟化策略的情况下，租户虚拟网络将数据包直接路由到 RAS 网关 Vm，而无需知道路由反射器) 然后通过站点到站点 VPN 隧道对企业站点进行了解 (。

此外： 从租户虚拟网络将流量返回到远程租户企业站点将绕过 SLBs，称为直接服务器 (DSR) 的过程。
...
由于可以根据需要将任意数量的 RAS 网关 Vm 添加到 RAS 网关池，因此可以轻松地扩展 RAS 网关部署以优化性能和容量。 将 Vm 添加到池时，你可以使用任何类型的站点到站点 VPN 连接来配置这些 RAS 网关 (IKEv2、L3、GRE) ，消除不停机的容量瓶颈。
...
若要确保快速故障转移响应，可以将边缘路由和控制路由器之间的 BGP Keepalive 参数时间配置为短时间间隔，例如小于或等于10秒。 使用此短暂的 keep-alive 间隔，如果 RAS 网关 BGP 边缘路由器发生故障，则会快速检测到故障，并按前面部分中提供的步骤进行操作。 此优势可能会减少单独的故障检测协议的需要，例如双向转发检测 (BFD) 协议。
