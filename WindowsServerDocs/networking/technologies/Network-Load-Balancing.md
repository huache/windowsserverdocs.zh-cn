---
title: 网络负载平衡
description: 本主题提供了 Windows Server 2016 中的网络负载平衡 \(NLB\) 功能的概述。 可以使用 NLB 将两个或多个服务器作为单个虚拟群集进行管理。 NLB 增强了 Internet 服务器应用程序的可用性和可伸缩性，如在 web、FTP、防火墙、代理、虚拟专用网络 \(VPN\)上使用的应用程序，以及其他任务\-关键服务器。
manager: dougkim
...
本主题提供了 Windows Server 2016 中的网络负载平衡 \(NLB\) 功能的概述。 可以使用 NLB 将两个或多个服务器作为单个虚拟群集进行管理。 NLB 增强了 Internet 服务器应用程序的可用性和可伸缩性，如在 web、FTP、防火墙、代理、虚拟专用网络 \(VPN\)上使用的应用程序，以及其他任务\-关键服务器。  

> [!NOTE]
> Windows Server 2016 提供了一个新的 Azure 创意软件负载平衡器 \(SLB\) 作为软件定义的网络 \(SDN\) 基础结构的组件。 使用 SLB 而不是 NLB。如果使用的是 SDN，使用的是非 Windows 工作负荷，\(NAT\)需要出站网络地址转换，或者需要第3层 \(L3\) 或基于非 TCP 的负载平衡。 对于非 SDN 部署，可以继续使用 NLB 和 Windows Server 2016。 有关 SLB 的详细信息，请参阅[SDN 的软件负载平衡（SLB）](../sdn/technologies/network-function-virtualization/Software-Load-Balancing--SLB--for-SDN.md)。
...
通过最大程度地减少停机时间，高可用性系统能够可靠地提供可接受级别的服务。 为了提供高可用性，NLB 包含了可自动执行的功能中的内置\-：  
...
-   平衡来自同一客户端的多个服务器加载请求，或者与群集中多个主机上\) 的多个客户端之间 \(。  
...
-   为每个网站定义不同的端口规则。 如果为多个应用程序或网站使用相同的负载\-平衡服务器，则端口规则基于目标虚拟 IP 地址 \(使用虚拟群集\)。  

-   通过使用可选的单一\-主机规则，将所有客户端请求定向到单个主机。 NLB 将客户端请求路由到运行特定应用程序的特定主机。  

-   阻止对某些 IP 端口进行不需要的网络访问。  

-   在群集主机上启用 Internet 组管理协议 \(IGMP\) 支持，控制交换机端口流 \(，其中传入的网络数据包会发送到交换机上的所有端口，\) 在多播模式下操作时。  
...
- 允许客户端使用单个逻辑 Internet 名称和虚拟 IP 地址（称为群集 IP 地址）访问群集，\(会保留每台计算机\)的各个名称。 NLB 允许多宿主服务器具有多个虚拟 IP 地址。  
...
-   如果使用单播模式，用于处理客户端\-\-群集通信的网络适配器必须支持更改其媒体访问控制 \(MAC\) 地址。  
  
## <a name="software-requirements"></a>软件要求  
下面是运行 NLB 群集的软件要求。  
  
-   只能在每个主机上启用 NLB 的适配器上使用 TCP\/IP。 不要添加任何其他协议 \(例如，IPX\) 此适配器。  
  
-   该群集中服务器的 IP 地址必须为静态地址。  
  
> [!NOTE]  
> NLB 不支持 \(DHCP\)的动态主机配置协议。 NLB 在其配置的每个接口上禁用 DHCP。  
