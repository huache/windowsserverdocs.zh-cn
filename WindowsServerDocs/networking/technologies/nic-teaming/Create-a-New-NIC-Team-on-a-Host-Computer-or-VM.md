...
description: 在本主题中，你将在主计算机上或在运行 Windows Server 2016 (VM) 的 Hyper-v 虚拟机中创建新的 NIC 组。
manager: dougkim
...
ms.locfileid: "87952392"
---
# <a name="create-a-new-nic-team-on-a-host-computer-or-vm"></a>在主计算机或 VM 上创建新的 NIC 组

>适用于：Windows Server（半年频道）、Windows Server 2016

在本主题中，你将在主计算机上或在运行 Windows Server 2016 (VM) 的 Hyper-v 虚拟机中创建新的 NIC 组。
...
物理交换机、Hyper-v 虚拟交换机、局域网 (LAN) 和 NIC 组合要求用于在 VM 中创建 NIC 组：
...
你还可以根据需要配置主团队界面并配置虚拟 LAN (VLAN) 号。
...
6. 在 "**其他属性**" 中，选择以下项的值：
   ...
   - **组合模式**。 分组模式选项是**独立交换**和**交换机依赖**的选项。 交换机依赖模式包含**静态组合**和**链接聚合控制协议 (LACP) **。
     ...
     - **依赖于交换机。** [!INCLUDE [switch-dependent-shortdesc-include](../../includes/switch-dependent-shortdesc-include.md)]
       ...
       | **链接聚合控制协议 (LACP) ** | 与静态组合不同，LACP 组合模式动态标识在主机和交换机之间连接的链接。 这种动态连接使团队能够自动创建，但在理论上，只是通过传输或接收来自对等实体的 LACP 数据包来扩展和减少团队。 所有服务器类交换机都支持 LACP，所有这些交换机都需要网络运营商在交换机端口上以管理方式启用 LACP。 配置 LACP 的分组模式时，NIC 组合始终在 LACP 的活动模式下运行。  默认情况下，NIC 组合使用短计时器 (3 秒) ，但你可以使用)  (90 秒来配置长计时器 `Set-NetLbfoTeam` 。 |
   ...
   - **备用适配器**。 备用适配器的选项为 "**无" (所有处于活动) 状态的适配器**，或在作为备用适配器的 NIC 组中选择的特定网络适配器。

   > [!TIP]
   > 如果要在虚拟机 (VM) 中配置 NIC 组，则必须选择 "_独立交换机_" 和 "_地址哈希_的**负载平衡" 模式**的 "**组合模式**"。
