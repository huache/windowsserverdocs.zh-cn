...
ms.locfileid: "80315734"
---
# <a name="plan-nps-as-a-radius-server"></a>将 NPS 规划为 RADIUS 服务器

>适用于：Windows Server（半年频道）、Windows Server 2016

将网络策略服务器 \(NPS 部署\) 为远程身份验证拨入用户服务（RADIUS）服务器时，NPS 将为本地域和信任本地域的域的连接请求执行身份验证、授权和记帐。 您可以使用这些规划指南来简化您的 RADIUS 部署。
...
例如，你可能想要为你的组织部署无线和 VPN 访问，但对于每种类型的访问，请使用不同的身份验证方法：针对 VPN 连接使用 EAP-TLS，因为 EAP 使用传输层安全性（EAP-TLS）提供了强大的安全性为 802.1 X 无线连接提供和 PEAP MS-CHAP v2。
...
对于 EAP-TLS，NPS 从证书颁发机构注册服务器证书 \(CA\)，证书将保存在本地计算机上的证书存储中。 在身份验证过程中，当 NPS 向访问客户端发送其服务器证书以向访问客户端证明其身份时，将发生服务器身份验证。 访问客户端将检查各种证书属性，以确定证书是否有效，以及在服务器身份验证期间是否适用于使用。 如果服务器证书满足最低服务器证书要求并且由访问客户端信任的 CA 颁发，则 NPS 已成功通过客户端的身份验证。
...
NPS 提供使用 SQL Server 日志记录来记录从一个或多个网络访问服务器接收的用户身份验证和记帐请求，并将其发送到运行 Microsoft SQL Server 桌面引擎 \(MSDE 2000\)的计算机上的数据源，或 SQL Server 2000 之前的任何 SQL Server 版本。

记帐数据从 XML 格式的 NPS 传递到数据库中的存储过程，该存储过程支持 \(SQL\) 和 XML \(SQLXML\)的结构化查询语言。 将用户身份验证和记帐请求记录到符合 XML 的 SQL Server 数据库中，可使多个 NPSs 具有一个数据源。
