...
>[!NOTE]
>如果使用的是 MS-CHAP v2 身份验证协议，则在使用连接请求策略转发 RADIUS 消息时，无法操作 "用户名" 属性。 仅当使用反斜杠（\) 字符，并且操作仅影响其左侧的信息时，才会发生例外。 反斜杠字符通常用于指示域名（反斜杠字符左侧的信息）以及域中的用户帐户名称（反斜杠字符右侧的信息）。 在这种情况下，只允许修改或替换域名的属性操作规则。
