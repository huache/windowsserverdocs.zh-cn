...
|检查 UAC 滑块级别|UAC 现在有四个通知级别可供选择，而且还有一个用于选择通知级别的滑块：<p><ul><li>高<p>    如果将滑块设置为“始终通知”****，则系统会检查安全桌面是否启用。</li><li>中<p>    如果将滑块设置为“默认 - 仅在程序尝试对我的计算机进行更改时通知我”****，则会检查“用户帐户控制: 只提升签名并验证的可执行文件”**** 策略设置。<p><ul><li>如果启用了该策略设置，则将对给定的可执行文件强制执行公钥基础结构 (PKI) 证书路径验证，然后才允许运行该文件。</li><li>如果未启用该策略设置（默认），则在允许运行给定的可执行文件之前，不会强制执行 PKI 证书路径验证。 系统而是会检查“用户帐户控制: 提示提升时切换到安全桌面”**** 组策略设置。</li></ul></li><li>低<p>    如果将滑块设置为“仅当程序尝试更改计算机时通知我(不降低桌面亮度)”****，则会调用 CreateProcess。</li><li>从不通知<p>    如果将滑块设置为 " **从不通知我**"，则当程序尝试安装或尝试在计算机上进行任何更改时，UAC 提示将永远不会发出通知。 **重要提示：**     不建议使用此设置。 此设置与设置 "**用户帐户控制：管理员批准模式中管理员的提升提示行为"** 策略设置中的**相同。**</li></ul>|
