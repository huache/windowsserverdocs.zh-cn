...
4. 执行诊断，在 `-Path` 参数上提供已汇编跟踪文件夹的路径，并指定 `-RunDiagnostics` 开关以及您要求管理员收集跟踪的诊断信息。  诊断将假定它无法访问在路径中找到的主机，因此将尝试仅使用预先收集的跟踪。  如果任何跟踪丢失或损坏，诊断将仅失败受影响的测试并正常运行。  例如：
