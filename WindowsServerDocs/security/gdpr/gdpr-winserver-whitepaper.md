...
如果控制流防护检查在运行时失败，Windows Server 2016 会立即终止程序，从而中断试图间接调用无效地址的任何攻击。 控制流防护为设备保护提供了一个重要的附加保护层。 如果已列出了允许列表的应用程序，则设备保护可以运行该应用程序，因为 Device Guard 筛选会发现该应用程序已签名并被视为受信任。
