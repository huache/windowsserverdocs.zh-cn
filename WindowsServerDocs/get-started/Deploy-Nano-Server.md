...
## <a name="nano-server-image-builder"></a>Nano Server 映像生成器
...
从[下载中心](https://www.microsoft.com/download/details.aspx?id=54065)获取工具。

该工具也需要 [Windows 评估和部署工具包 (ADK)](https://developer.microsoft.comwindows/hardware/windows-assessment-deployment-kit)。
...
## <a name="creating-a-custom-nano-server-image"></a><a name=BKMK_CreateImage></a>创建自定义 Nano Server 映像
...
此表显示了此版本的 Nano Server 中可用的角色和功能，以及将为其安装包的 Windows PowerShell 选项。 某些包可直接使用其自身的 Windows PowerShell 开关进行安装（例如 -Compute）；其他包需通过将其名称传递到 -Package 参数进行安装（可在由逗号分隔的列表中进行组合）。 可使用 Get-NanoServerPackage cmdlet 动态列出可用程序包。
...
### <a name="failover-clustering-items-installed-by-the--clustering-parameter"></a>由 -Clustering 参数安装的故障转移群集项
...
### <a name="file-and-storage-items-installed-by-the--storage-parameter"></a>由 -Storage 参数安装的文件和存储项
...
### <a name="installing-a-nano-server-vhd"></a>安装 Nano Server VHD
...
该 cmdlet 将在运行时生成日志并在完成后告知日志的位置。 由配套脚本完成的 WIM-to-VHD 转换会在 %TEMP%\Convert-WindowsImage\\\<GUID> 中生成自己的日志（其中，\<GUID> 为每个转换会话的唯一标识符）。
...
### <a name="installing-a-nano-server-wim"></a>安装 Nano Server WIM
...
连接到 Nano Server 后，可通过将文件的相对和绝对路径传递到 psEdit 命令编辑驻留在本地计算机上的文件，例如：`psEdit C:\Windows\Logs\DISM\dism.log` 或 `psEdit .\myScript.ps1`

通过使用 `Enter-PSSession -ComputerName 192.168.0.100 -Credential ~\Administrator` 启动远程会话，编辑驻留在远程 Nano Server 上的文件，然后将文件的相对和绝对路径传递到 psEdit 命令，如下所示：`psEdit C:\Windows\Logs\DISM\dism.log`
...
还可使用通用 PackageManagement cmdlet 并且指定 NanoServerPackage 提供程序：
...
### <a name="searching-for-nano-server-packages"></a>搜索 Nano Server 程序包
...
若需要特定区域设置版本，如美国英语，可使用 `Find-NanoServerPackage -Culture en-us`、`Find-Package -ProviderName NanoServerPackage -Culture en-us` 或 `Find-Package -Culture en-us -DisplayCulture`。
...
### <a name="inventory-installed-packages"></a>已安装清单的程序包
...
### <a name="installing-roles-and-features-from-local-source"></a>从本地源安装角色和功能
...
4. 使用新创建的 XML 文件切换到目录并运行：`dism /online /apply-unattend:.\unattend.xml`

5. 请确定程序包及其关联的语言包已正确安装，方法是运行：`dism /online /get-packages`
...
可使用 Edit-NanoServerImage cmdlet 更改现有 VHD 的详细信息，如本示例所示：`Edit-NanoServerImage   -BasePath .\Base   -TargetPath .\BYOVHD.vhd`
...
## <a name="additional-tasks-you-can-accomplish-with-new-nanoserverimage-and-edit-nanoserverimage"></a>可使用 New-NanoServerImage 和 Edit-NanoServerImage 完成的其他任务

### <a name="joining-domains"></a>加入域
...
### <a name="adding-additional-drivers"></a>添加其他驱动程序
...
9. 在提升的命令提示符中，导航到 Nano Server VHD 所在目录并运行以下命令：
...
### <a name="injecting-drivers"></a>注入驱动程序
...
> [!NOTE]
> 保存驱动程序的文件夹中必须存在 SYS 文件和相应的 INF 文件。 而且，Nano Server 仅支持已签名、64 位的驱动程序。
...
### <a name="setting-static-ip-addresses"></a>设置静态 IP 地址
...
### <a name="custom-image-size"></a>自定义映像大小
...
### <a name="embedding-custom-data"></a>嵌入自定义数据
...
### <a name="running-custom-commands-after-the-first-boot"></a>第一次启动后运行自定义命令
...
### <a name="running-custom-powershell-scripts-as-part-of-image-creation"></a>运行自定义 PowerShell 脚本作为映像创建的一部分
...
### <a name="support-for-development-scenarios"></a>对开发方案的支持
...
### <a name="custom-unattend-file"></a>自定义无人参与文件
...
### <a name="collecting-log-files"></a>收集日志文件

如果想要在映像创建过程中收集日志文件，请使用 -LogPath 参数指定所有日志文件复制到的目录。
...
> [!NOTE]
> New-NanoServerImage 和 Edit-NanoServerImage 上的某些参数仅供内部使用，因此可以放心地忽略。 这些参数包括 -SetupUI 和 -Internal 参数。
...
创建 Panther 文件夹（Windows 系统使用该文件夹在安装过程中存储文件；如感兴趣，请参阅 [Windows 7、Windows Server 2008 R2 和 Windows Vista 安装日志文件位置](https://support.microsoft.com/kb/927521)，将 Unattend.xml 文件复制到该文件夹，然后使用以下命令卸载 VHD：
...
## <a name="working-with-server-roles-on-nano-server"></a>使用 Nano Server 上的服务器角色

### <a name="using-hyper-v-on-nano-server"></a>使用 Nano Server 上的 Hyper-V
...
- [启用 Hyper-V 远程管理 - 为 SMB 和高度可用的 SMB 配置约束的委派](https://blogs.msdn.com/b/taylorb/archive/2012/03/20/enabling-hyper-v-remote-management-configuring-constrained-delegation-for-smb-and-highly-available-smb.aspx)

- [启用 Hyper-V 远程管理 - 为非群集的实时迁移配置约束的委派](https://blogs.msdn.com/b/taylorb/archive/2012/03/20/enabling-hyper-v-remote-management-configuring-constrained-delegation-for-non-clustered-live-migration.aspx)

CredSSP

首先参阅本主题的“使用 Windows PowerShell 远程处理”部分以启用和测试 CredSSP。 然后，可在管理计算机上使用 Hyper-V 管理器，并选择“以其他用户身份连接”选项。 Hyper-V 管理器将使用 CredSSP。 即使使用的是当前帐户，也须执行此操作。

Hyper-V 的 Windows PowerShell cmdlet 可使用 CimSession 或 Credential 参数，两者都可使用 CredSSP。

### <a name="using-failover-clustering-on-nano-server"></a><a name=BKMK_Failover></a>使用 Nano Server 上的故障转移群集
...
### <a name="using-iis-on-nano-server"></a><a name=BKMK_IIS></a>使用 Nano Server 上的 IIS
...
### <a name="using-ssh-on-nano-server"></a><a name=BKMK_SSH></a>使用 Nano Server 上的 SSH
