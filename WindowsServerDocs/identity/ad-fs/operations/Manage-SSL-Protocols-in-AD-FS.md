...
传输层安全性 (TLS) ，安全套接字层 (SSL) 是提供安全通信的协议。  Active Directory 联合身份验证服务使用这些协议进行通信。  目前有几个版本的协议。
...
有关支持的密码套件的完整列表，请参阅[TLS/SSL 中的密码套件 (SCHANNEL SSP) ](/windows/win32/secauthn/cipher-suites-in-schannel)。  本文档提供默认情况下启用的套件的表以及支持但默认情况下未启用的套件。  若要确定密码套件的优先级，请参阅[优先级 Schannel 密码套件](/windows/win32/secauthn/prioritizing-schannel-cipher-suites)。
...
- [TLS/SSL 中的密码套件 (Schannel SSP) ](/windows/win32/secauthn/cipher-suites-in-schannel)
