...
6) 此外，对于 Exchange ActiveSync 客户端，Azure AD 对于 Exchange ActiveSync 客户端，客户端证书必须在 "使用者可选名称" 字段的主体名称或 RFC822 名称值中具有用户可路由电子邮件地址。  (Azure Active Directory 将 RFC822 值映射到目录中的代理地址属性。 ) 
...
>[!IMPORTANT]
> 客户使用设备代码流进行身份验证，并使用 Azure AD 以外的其他 IDP 执行设备身份验证 (例如，AD FS) 将不能强制实施基于设备的访问 (例如，仅允许使用第三方 MDM 服务) 托管设备 Azure AD 资源。 若要在 Azure AD 中保护对公司资源的访问并防止任何数据泄露，客户应配置基于 Azure AD 设备的条件访问 (例如 "要求设备标记为投诉" Azure AD 条件访问) 中授予控制权限。
...
- 你可能想要在执行证书身份验证时，根据最终用户的需求来修改登录页。 常见的情况是 () 更改 "使用 X509 证书登录"，以使最终用户更友好
...
如果在满足客户端身份验证目的的计算机上存在多个用户证书 (例如 Wi-fi 证书) ，则 Windows 桌面上的 Chrome 浏览器将提示用户选择正确的证书。 这可能会给最终用户造成混淆。 若要优化此体验，你可以将 "Chrome" 策略设置为自动选择正确的证书，以获得更好的用户体验。 此策略可通过以下方式进行手动设置：更改注册表或通过 GPO 自动配置 (设置) 的注册表项。 这需要你的用户客户端证书进行身份验证，以使不同的颁发者不同于其他用例的颁发者 AD FS。
...
AD FS 使用基础 windows 操作系统来证明用户证书的所有权，并通过执行证书信任链验证来确保它与受信任的颁发者匹配。 若要匹配受信任的颁发者，你将需要确保所有根和中间颁发机构都配置为本地计算机证书颁发机构存储中的受信任颁发者。
若要自动验证此情况，请使用[AD FS 诊断分析器工具](https://adfshelp.microsoft.com/DiagnosticsAnalyzer/Analyze)。 该工具将查询所有服务器，并确保正确配置正确的证书。
...
### <a name="check-if-certificate-authentication-is-enabled-in-the-ad-fs-authentication-policy"></a>检查是否已在 AD FS Authentication 策略中启用证书身份验证
默认情况下，AD FS 在端口49443上进行用户证书身份验证，其主机名与 AD FS (例如 `adfs.contoso.com`) 。 你还可以将 AD FS 配置为使用端口 443 (默认 HTTPS 端口) 使用备用 SSL 绑定。 但是，在此配置中使用的 URL `certauth.<adfs-farm-name>` (例如 `certauth.contoso.com`) 。 有关详细信息，请参阅[此链接](ad-fs-support-for-alternate-hostname-binding-for-certificate-authentication.md)。
网络连接最常见的情况是防火墙配置不正确，阻止或干扰用户证书身份验证流量。 通常情况下，出现此问题时，你将看到一个空白屏幕或500服务器错误。
1)  请注意你在中配置的主机名和端口 AD FS
2)  确保 AD FS 或 Web 应用程序代理 (WAP) 前面的任何防火墙都配置为允许 `hostname:port` AD FS 场的组合。 需要参考网络工程师才能执行此步骤。
...
每个 AD FS 和 WAP 服务器都需要到达 CRL 终结点，以验证提供给它的证书是否仍然有效且未被吊销。 可以通过 HTTPS、HTTP、LDAP 或通过 OCSP (Online 证书状态协议) 进行 CRL 验证。 如果 AD FS/WAP 服务器无法连接到终结点，则身份验证将失败。 请按照以下步骤进行故障排除。
1) 咨询 PKI 工程师，确定用于从 PKI 系统吊销用户证书的 CRL 终结点。
2)  在每个 AD FS/WAP 服务器上，确保可通过通常使用 HTTPS 或 HTTP (的协议访问 CRL 终结点) 
...
4) 在 CAPI2 操作日志中检查事件 ID 41 (验证吊销) 
...
***提示***：可以通过将 Windows) 上的 DNS 解析 (主机文件配置为指向特定服务器来更轻松地定位单个 AD FS 或 WAP 服务器。 这允许你启用针对服务器的跟踪。
...
2)  如果此问题特定于 Windows 设备，请检查是否正确设置了证书，方法是检查登录用户的 Windows 证书存储区 (不是系统/计算机) 。
...
在极少数情况下，客户端设备 (通常会将移动设备) 更新为仅支持更高版本的 TLS (说 1.3) ，否则可能会遇到相反的问题，即 AD FS/WAP 服务器更新为仅使用较高的 TLS 版本，客户端设备不支持。
可以使用联机 SSL 工具检查 AD FS/WAP 服务器，并查看其是否与设备兼容。
有关如何控制 TLS 版本的详细信息，请参阅[此链接](manage-ssl-protocols-in-ad-fs.md)。
