...
AD FS 2016 在向请求者进行身份验证时，将更改 PSSO，从注册的设备进行身份验证，增加到90天，但要求在14天内进行身份验证 (设备使用情况窗口) 。
...
默认情况下，"设备使用情况" 窗口 (14 天) 由 AD FS 属性**DeviceUsageWindowInDays**控制。

``` powershell
Set-AdfsProperties -DeviceUsageWindowInDays
```
默认情况下，最大单一登录周期 (90 天) 由 AD FS 属性**PersistentSsoLifetimeMins**控制。
...
对于非注册设备，单一登录期间由 "**使我保持登录 (KMSI") **功能设置确定。  默认情况下，KMSI 处于禁用状态，可以通过将 AD FS 属性 KmsiEnabled 设置为 True 来启用。
...
请务必注意，尽管提供相对较长时间的单一登录，AD FS 会提示进行其他身份验证 (多重身份验证) 当上一次登录基于主要凭据而不是 MFA 时，但当前登录需要 MFA。  这与 SSO 配置无关。 AD FS，当接收到身份验证请求时，将首先确定是否存在 SSO 上下文 (例如 cookie) ，然后，如果需要 MFA (例如，如果请求来自) 外部，则会评估 SSO 上下文是否包含 MFA。  否则，系统会提示 MFA。
...
之所以发生这种情况，是因为 Azure AD 无法确定何时吊销与旧凭据（例如已) 更改的密码）相关的令牌 (。 因此，Azure AD 必须更频繁地进行检查，以确保用户和关联的令牌仍处于良好地位。
