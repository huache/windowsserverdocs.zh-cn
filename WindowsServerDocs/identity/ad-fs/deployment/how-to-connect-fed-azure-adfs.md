...
* **DC/ADFS 服务器**：如果用户数少于1000，只需在域控制器上安装 AD FS 角色即可。 如果你不希望对域控制器产生任何性能影响，或者如果你的用户超过1000，则在单独的服务器上部署 AD FS。
...
* **存储帐户**：建议创建两个存储帐户。 使用单个存储帐户可能会导致创建单点故障，并可能导致在存储帐户出现故障的情况下，部署无法使用。 两个存储帐户将帮助每个错误行关联一个存储帐户。
* **网络隔离**：应将 Web 应用程序代理服务器部署在不同的外围网络中。 可以将一个虚拟网络分割成两个子网，然后在隔离的子网中部署 Web 应用程序代理服务器。 只需为每个子网配置网络安全组设置，并只允许在两个子网之间进行所需的通信。 下面是针对每个部署方案提供的详细信息
...
出于本指南的目的，我们将创建两个 Nsg：分别用于内部网络和外围网络。 它们将分别标记 NSG_INT 和 NSG_DMZ。
...
创建 Nsg 后，将 NSG_INT 与子网 INT and NSG_DMZ 与子网 DMZ 关联。 下面提供了示例屏幕截图：
...
* **更新域**：在更新期间，属于同一更新域的计算机将一起重新启动。 需要至少2个更新域。 默认值为5，你可以将其保留为用于此部署的目的
...
|:---:|:---:|:---:|:---:|:---:|:---:|
| contosodc1 |DC/ADFS |整形 |contosodcset |contososac1 |静态 |
| contosodc2 |DC/ADFS |整形 |contosodcset |contososac2 |静态 |
...
1. 单击 "添加"
2. 提供探测的详细信息。 **名称**：探测名称 b。 **协议**： HTTP c。 **端口**：80（HTTP） d。 **路径**：/adfs/probe e。 **间隔**：5（默认值）–这是 ILB 在后端池 f 中探测计算机的时间间隔。 不**正常阈值限制**：2（默认值）–这是连续探测失败阈值，超过此阈值后，ILB 会将后端池中的计算机声明为无响应，并停止向它发送流量。
...
2. 单击 "负载均衡规则" 面板中的 "添加"
3. 在 "添加负载均衡规则" 面板中。 **名称**：提供规则 b 的名称。 **协议**：选择 TCP c。 **端口**： 443 d。 **后端端口**： 443 e。 **后端池**：选择为之前 f 的 AD FS 群集创建的池。 **探测**：选择前面为 AD FS 服务器创建的探测
