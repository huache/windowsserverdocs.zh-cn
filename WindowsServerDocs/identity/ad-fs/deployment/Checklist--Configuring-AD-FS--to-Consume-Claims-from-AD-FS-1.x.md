...
|![使用从 AD FS 声明](media/icon_checkboxo.gif)|在之前创建的声明提供方信任上，必须创建一个声明规则，该规则将接受从 AD FS 1.x 联合身份验证服务传入的声明，并传递、筛选或转换为名称 ID 声明类型。<br /><br />当名称 ID 声明类型传递，通过筛选，或转换，它可以用作输入到另一个规则或规则，以便它可以理解并由 Windows Server 2012 中 AD FS 联合身份验证服务。|![使用来自 AD FS 的声明](media/faa393df-4856-4431-9eda-4f4e5be72a90.gif)[创建一个规则以发送 AD FS 1.X 兼容声明](../../ad-fs/operations/Create-a-Rule-to-Send-an-AD-FS-1x-Compatible-Claim.md)|  
