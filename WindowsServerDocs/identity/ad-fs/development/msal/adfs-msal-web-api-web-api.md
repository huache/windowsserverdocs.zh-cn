---
title: 'AD FS 代表方案调用 Web API (的 MSAL Web API) '
...
ms.locfileid: "87965114"
---
# <a name="scenario-web-api-calling-web-api-on-behalf-of-scenario"></a>方案：代表方案调用 web api (的 Web API) 
...
本部分演示如何将本机应用作为公共客户端和 Web Api 注册为依赖方 (RP) 在 AD FS
  ...
  28. 在**声明规则名称：** 字段和**x： [] => 发出 (声明 = x) ;** 声明规则在自定义规则：字段中输入**PassAllClaims** ，然后单击 "完成"。
  ...
  33. 在声明规则名称：字段和**x： [] => 发出 (声明 = x) ;** 声明规则在**自定义规则：** 字段中输入 PassAllClaims，然后单击 "**完成**"。
...
更改代码后，重新生成解决方案
  ...
  6. 登录后，在 "创建待办事项"**项**中的 "Web api 调用" 中输入文本 web api。 单击 "**添加项**"。  这会调用 Web API (待办事项列表服务，) 随后会调用 Web API 2 (WebAPIOBO) 并将该项添加到缓存中。
