...
- **使用智能卡登录的远程 PowerShell (PSH)** - 客户现在可以使用智能卡通过 PSH 远程连接到 ADFS，并使用它来管理所有 PSH 功能，包括多节点 PSH cmdlet。
...
> [!NOTE]
> 可能会在 ADFS 管理事件日志中遇到此错误：收到的 Oauth 请求无效。 禁止客户端 'NAME' 访问作用域为 'ugs' 的资源。
> 若要修正此错误，请执行以下操作：
> 1. 启动 AD FS 管理控制台。 浏览到“服务 > 范围说明”
...
**Q.** 是否可以像在 Azure AD 中完成请求那样将资源值作为作用域值的一部分进行传递？</br>
**A.** 借助 Server 2019 上的 AD FS，你现在可以传递嵌入在 scope 参数中的资源值。 现在可以将 scope 参数组织成一个用空格分隔的列表，其中每个条目的结构都作为资源/范围。 例如 <创建有效的示例请求>

**Q.** AD FS 是否支持 PKCE 扩展？</br>
**A.** Server 2019 中的 AD FS 支持适用于 OAuth 授权代码授予流的代码交换证明密钥 (PKCE)
...
要查找有关 AD FS 早期版本的信息，请参阅以下文章：[Windows Server 2012 或 2012 R2 中的 ADFS](../../active-directory-federation-services.md) 和 [AD FS 2.0](/previous-versions/windows/it-pro/windows-server-2008-r2-and-2008/dd727958(v=ws.10))
...
*  [配置 AD FS 2016 和 Azure MFA](../operations/configure-ad-fs-and-azure-mfa.md)
 ...
 * [Azure Active Directory 条件访问](/azure/active-directory/conditional-access/overview)
...
* [在组织中启用 Windows Hello 企业版](/windows/security/identity-protection/hello-for-business/hello-identity-verification)
...
有关详细信息，请参阅[升级到 Windows Server 2016 中的 AD FS](../deployment/upgrading-to-ad-fs-in-windows-server.md)。
