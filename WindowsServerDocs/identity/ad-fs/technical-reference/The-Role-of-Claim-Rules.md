...
|模拟授权规则集|可以使用 Windows PowerShell 配置的声明规则集，用于确定用户是否可以向依赖方完全模拟其他用户。<p>这些规则可确定是否允许请求者模拟用户，而不在发送到信赖方的令牌中标识请求者。<p>采用这种方式模拟另一位用户是一项非常强大的功能，因为信赖方不知道被模拟的用户。|依赖方信任 (Relying party trust)|
