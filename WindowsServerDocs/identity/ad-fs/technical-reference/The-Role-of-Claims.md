...
|给定名称|用户的给定名称|http：\/\/schemas.xmlsoap.org\/ws\/2005\/05\/\/声明\/givenname|  
...
每个声明说明包括声明类型 URI、名称、发布状态和描述。 您可以使用中的 "AD FS 管理" 管理单元\-中的 "**声明说明**" 节点管理声明说明集合。 您可以使用中的 snap\-修改声明说明的发布状态。 提供了下列设置：  
