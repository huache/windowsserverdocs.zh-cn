...
ms.locfileid: "80860200"
---
# <a name="how-uris-are-used-in-ad-fs"></a>在 AD FS 中如何使用 URI
URI\) \(的统一资源标识符是用作唯一标识符的字符串。  在 AD FS 中，URI 用于标识合作伙伴网络地址和配置对象。  用于标识合作伙伴网络地址时，URI 始终是 URL。  用于标识配置对象时，URI 可以是 URN，也可以是 URL。  有关 URI 的更多常规信息，请参阅 [RFC 2396](https://go.microsoft.com/fwlink/?LinkId=48289) 和 [RFC 3986](https://go.microsoft.com/fwlink/?LinkId=90453)。  
  
## <a name="uris-as-partner-network-addresses"></a>用作合作伙伴网络地址的 URI  
以下是 AD FS 中的管理员最常处理的网络地址 URL。  
  
-   联合身份验证服务的 Url，包括 WS\-Federation、SAML、WS\-信任、联合元数据、WS\-Ws-metadataexchange、隐私和组织 Url  
...
|信赖方标识符|此标识符用于标识此联合身份验证服务的信赖方。  在向信赖方发出声明时使用。|当用户从此联合身份验证服务请求有关信赖方的声明时，信赖方标识符将用于标识应作为声明目标的信赖方。  此比较是使用前缀匹配完成的 \(参见下面\)。<p>当信赖方收到声明时，它将检查其在安全令牌中的标识符，以确保声明的目标是它。|  
...
URI 的路径语法按层次结构组织，并由所有 "\/" 字符或所有 "：" 字符分隔。  因此，该路径可以基于分隔字符拆分为多个路径部分。  当前缀匹配时，每个部分必须与匹配规则完全匹配 \(这些规则控制\)的匹配大小写。 有关匹配规则的详细信息，请参阅上文所述的 RFC。  
  
在向联合身份验证服务发出的请求中标识信赖方时，AD FS 将使用前缀匹配逻辑来确定 AD FS 配置数据库中是否有匹配的信赖方信任。  
  
例如，如果 AD FS 配置数据库中的信赖方标识符 \(URI1\) 是传入请求 \(URI2\)中的信赖方标识符的前缀，则必须满足以下条件：  
  
-   必须忽略路径部分或授权的尾部分隔符 \(斜杠和冒号\)  
  
-   URI1 和 URI2 的方案和授权部分必须完全匹配（不区分大小写）  
  
-   URI1 的每个路径部分都必须是完全匹配 \(基于\) 所选的区分大小写到 URI2 的相应路径部分。  
...
|http：\/\/contoso.com|http：\/\/contoso.com\/|TRUE|忽略尾部斜杠|  
|http：\/\/contoso.com|http：\/\/contoso.com\/小时|TRUE|URI1 没有路径，且方案和授权与 URI2 匹配|  
|http：\/\/contoso.com\/小时|http：\/\/contoso.com\/hr\/web|TRUE|第一个路径部分匹配，URI1 没有第二个路径部分|  
|http：\/\/contoso.com\/小时|http：\/\/contoso.com\/hr\/web\/？ m\=t|TRUE|如上所述，查询字符串不会更改任何内容|  
|http：\/\/contoso.com\/hr\/|http：\/\/contoso.com\/hrw\/main|FALSE|URI1 路径部分 1 与 URI2 路径部分 1 不匹配|  
|http：\/\/contoso.com\/小时|http：\/\/contoso.com|FALSE|URI1 的路径部分比 URI2 多|  
|http：\/\/contoso.com\/小时|http：\/\/contoso.com\/hrweb|FALSE|第一个路径部分不匹配|  
