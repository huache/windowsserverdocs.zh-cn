...
|5027|不适用|中型|Windows 防火墙服务无法从本地存储中检索安全策略。 该服务将继续强制实施当前策略。|
...
|5042|不适用|低|已对 IPsec 设置进行了更改。 已删除身份验证集。|
