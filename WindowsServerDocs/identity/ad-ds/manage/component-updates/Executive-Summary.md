...
- **用于凭据盗用**的有吸引力的帐户-凭据被盗攻击是指攻击者最初获得网络中计算机的特权访问权限，然后使用自由可用工具从其他登录帐户的会话中提取凭据。 本部分包括以下内容：
