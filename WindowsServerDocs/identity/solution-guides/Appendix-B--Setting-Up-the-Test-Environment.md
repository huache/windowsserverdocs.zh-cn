...
2.  根据指定的详细信息创建以下文件：
    ...
    -   在桌面上创建称为“正则表达式”的文件夹。 在称为“RegEx-SSN”**** 的文件夹下创建一个文本文档。 在文件中键入以下内容，然后保存并关闭文件： ^ (？！000) # A2 [0-7] \d {2} | 7 ( [0-7] \d | 7 [012] ) # A5 # A6 [-]？ ) # A8？！00) \d\d\3 (？0000) \d {4} $
...
19. 在 "**服务帐户**" 屏幕上的 "**域用户帐户**" 中，单击 "**指定**"，然后指定 "用户名 (**contoso\rms**) " 和 "密码" () ，然后 <strong>pass@word1</strong> 单击 **"确定**"，然后单击 "**下一步**"。
...
28. 在“结果”**** 屏幕上，单击“关闭”****，然后在“安装进度”**** 屏幕上，单击“关闭”****。 完成后，请使用)  (提供的密码注销并登录 <strong>pass@word1</strong> 。
...
7. 以 CONTOSO\Administrator 的身份注销，并使用提供的密码 ( "" ) 登录为 CONTOSO\RMS pass@word1 。
...
### <a name="set-contosocom-as-a-trusting-forest-to-adatumcom"></a><a name="BKMK_2.22"></a>将 contoso.com 设置为 adatum.com 的信任林
