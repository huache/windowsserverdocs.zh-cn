...
## <a name="step-1-enable-resource-properties"></a><a name="BKMK_1.1"></a>步骤 1：启用资源属性

#### <a name="to-enable-resource-properties"></a>启用资源属性

1. 在 Hyper-V 管理器中，连接到服务器 ID_AD_DC1。 使用 Contoso\Administrator 和密码登录到服务器 <strong>pass@word1</strong> 。
...
## <a name="step-2-create-classification-rules"></a><a name="BKMK_2"></a>步骤 2：创建分类规则
...
1. 在 Hyper-V 管理器中，连接到服务器 ID_AD_FILE1。 使用 Contoso\Administrator 和密码登录到服务器 <strong>pass@word1</strong> 。
...
2. 在桌面上，打开名为“正则表达式”**** 的文件夹，然后打开名为“RegEx-SSN”**** 的文本文档。 突出显示并复制以下正则表达式字符串： **^ (？！000) # A2 [0-7] \d {2} | 7 ( [0-7] \d | 7 [012] ) # A5 # A6 [-]？ ) # A8？！00) \d\d\3 (？0000) \d {4} $**。 在本步骤的后面部分会使用此字符串，因此请将它保留在你的剪贴板上。
...
11. 在“参数”**** 下单击“配置”****。
    在 "**分类参数**" 窗口的 "**表达式类型**" 列表中，选择 "**正则表达式**"。 在 "**表达式**" 框中，粘贴剪贴板中的文本： **^ (？！000) # A2 [0-7] \d {2} | 7 ( [0-7] \d | 7 [012] ) # A5 # A6 [-]？ ) # A8？！00) \d\d\3 (？0000) \d {4} $**，然后单击 **"确定"**。
...
## <a name="step-3-use-file-management-tasks-to-automatically-protect-documents-with-ad-rms"></a><a name="BKMK_3"></a>步骤 3：使用文件管理任务来通过 AD RMS 自动保护文档
...
## <a name="step-4-view-the-results"></a><a name="BKMK_4"></a>步骤 4：查看结果
...
6. 切换到 CLIENT1。 注销任何已登录的用户，然后以 Contoso\MReid 的身份登录，并提供密码 <strong>pass@word1</strong> 。

7. 从桌面上，打开“财务文档”**** 共享文件夹。

8. 打开“财务备注”**** 文档。 在接近文档底部处，会看到**机密**一词。 将其修改为： **Contoso 机密**。 保存该文档并关闭。

9. 打开“请求批准聘用”**** 文档。 在“身份证号:” **** 部分中，键入：777-77-7777。 保存该文档并关闭。
...
## <a name="step-5-verify-protection-with-ad-rms"></a><a name="BKMK_5"></a>步骤 5：验证使用 AD RMS 的保护
