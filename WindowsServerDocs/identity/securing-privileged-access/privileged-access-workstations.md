...
* **供应链篡改** - 虽然不可能清除硬件和软件供应链中所有可能的篡改风险，但是执行一些关键操作可以缓解攻击者现已可用的关键攻击媒介。 这些操作包括验证所有安装介质的完整性（[清洁源原则](./securing-privileged-access-reference-material.md)），以及使用受信任且信誉好的供应商提供的硬件和软件。
...
在 Windows 10 中引入的[凭据保护](/windows/security/identity-protection/credential-guard/credential-guard)使用基于硬件和虚拟化的安全性，通过保护派生的凭据来缓解常见的凭据被盗攻击（例如哈希传递）。 [Windows Hello 企业版](/windows/security/identity-protection/hello-for-business/hello-identity-verification)所使用凭据的私钥还可以通过受信任的平台模块 (TPM) 硬件进行保护。
...
Microsoft 建议使用 PAW 访问特权管理解决方案。 应仅向 PAW 授予这些解决方案的访问权限。 Microsoft 不建议将这些解决方案用作 PAW 的替代方案，因为从可能被入侵的用户桌面使用这些解决方案访问特权违反了如下图中所示的[清洁源](./securing-privileged-access-reference-material.md)原则：
...
> 有关层模型的详细信息，请参阅 [https://aka.ms/tiermodel](./securing-privileged-access-reference-material.md)。有关第 0 层组的详细信息，请参阅[保护特权访问参考资料](../securing-privileged-access/securing-privileged-access-reference-material.md)中第 0 层的等效性。

有关部署 Microsoft 标识管理器 (MIM) 特权访问管理 (PAM) 的详细信息，请参阅 [https://aka.ms/mimpamdeploy](/microsoft-identity-manager/pam/privileged-identity-management-for-active-directory-domain-services)
...
请参阅[层模型页](./securing-privileged-access-reference-material.md)，获取有关层指定的详细信息。
...
4. **获取并验证所需的 Windows 10 企业版及应用程序软件**。 获取 PAW 所需的软件，并使用[安装介质的清洁源](./securing-privileged-access-reference-material.md)中的指南进行验证。

   * Windows 10 企业版
   * 适用于 Windows 10 的 [远程服务器管理工具](https://www.microsoft.com/download/details.aspx?id=45520)
   * [Windows 10 安全基线](/windows/security/threat-protection/windows-security-baselines)

      > [!NOTE]
      > Microsoft 在 MSDN 上为所有操作系统和应用程序发布了 MD5 哈希值，但并不是所有软件供应商都提供类似文档。  在这些情况下将需要其他策略。  有关验证软件的其他信息，请参阅[安装介质的清洁源](./securing-privileged-access-reference-material.md)。
...
1. 使用先前获取的清洁源安装介质安装 Windows 10。
   ...
   > 如果自动部署 PAW，你必须：
   >
   > * 使用经[安装介质的清洁源](./securing-privileged-access-reference-material.md)验证的安装介质构建系统。
...
6. 使用 PAW 维护组成员的帐户，从新创建的 PAW 运行以下 PowerShell 命令，以将其加入到相应的 OU 中的域中：
   ...
   > [!NOTE]
   > 如果你已部署了 [ESAE 管理林](./securing-privileged-access-reference-material.md)（适用于阶段 1 中的第 0 层管理员）或 [Microsoft 标识管理器 (MIM) 特权访问管理 (PAM)](/microsoft-identity-manager/pam/privileged-identity-management-for-active-directory-domain-services)（适用于阶段 2 中的第 1 层和第 2 层管理员），你应把 PAW 联接到此处环境中的域，而不是生产域中。
...
9. （可选）为 Active Directory 管理员安装其他所需的工具。 安装执行作业职责所需的任何其他工具或脚本。 将凭据添加到 PAW 之前，确保使用任意工具评估凭据在目标计算机上泄露的风险。 访问[此页面](./securing-privileged-access-reference-material.md)获取评估凭据暴露风险的管理工具和连接方法的详细信息。 确保使用[安装介质的清洁源](./securing-privileged-access-reference-material.md)中的指南获取全部安装介质。
...
6. （可选）为第 1 层管理员安装其他所需的工具。 安装执行作业职责所需的任何其他工具或脚本。 将凭据添加到 PAW 之前，确保使用任意工具评估凭据在目标计算机上泄露的风险。 有关评估凭据暴露风险的管理工具和连接方法的详细信息，请访问[此页面](./securing-privileged-access-reference-material.md)。 确保使用“安装介质的清洁源”中的指南获取全部安装介质
...
PAW 解决方案应使用基于清洁源原则的[操作标准](./securing-privileged-access-reference-material.md)中的标准进行操作。

## <a name="deploy-paws-using-a-guarded-fabric"></a>使用受保护的构造部署 PAW

[受保护的构造](../../security/guarded-fabric-shielded-vm/guarded-fabric-and-shielded-vms-top-node.md)可用于在便携式计算机或跳转服务器上的受防护虚拟机中运行 PAW 工作负荷。
采用此方法需要额外的基础结构和操作步骤，但可以更轻松地按固定时间间隔重新部署 PAW 映像，并可以将多个不同的分层（或分类）PAW 整合到单个设备上一起运行的虚拟机。
有关受保护构造的拓扑和安全承诺的完整说明，请参阅[受保护的构造文档](../../security/guarded-fabric-shielded-vm/guarded-fabric-and-shielded-vms-top-node.md)。
