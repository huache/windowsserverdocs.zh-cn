...
|工业控制系统（例如 SCADA、PCN 和 DCS）|部分|使用阶段 2 中提供的指南构建的 PAW 可作为起点，为这些角色提供安全性，因为与大多数 ICS 控制台（包括 SCADA 和 PCN 等通用标准）不需要浏览开放的 Internet 及检查电子邮件。<br /><br />- 用于控制物理机器的应用程序必须正确进行集成、兼容性测试并受到保护。|
