...
|选项  |描述  |
|---------|---------|
|服务器复原能力和负载均衡     |在需要高可用性或支持大量请求的环境中，可以通过在运行网络策略服务器的多个服务器 (NPS) 并启用远程访问服务器群集来提高远程访问的性能和复原能力。<p>相关文档：<ul><li>[NPS 代理服务器负载平衡](../../../../../networking/technologies/nps/nps-manage-proxy-lb.md)</li><li>[在群集中部署远程访问](../../../ras/cluster/deploy-remote-access-in-cluster.md)</li></ul>        |
...
1. 搜索包含以下信息的事件：
   ...
   * 事件 ID： **41**
   * 此事件包含以下文本： **subject = "*客户端 fqdn*"** (*客户端 fqdn*表示具有吊销的证书的客户端的完全限定的域名。 ) 
...
- [Tpm 密钥证明](../../../../../identity/ad-ds/manage/component-updates/tpm-key-attestation.md)：本主题概述了受信任的平台模块 (tpm) 和部署 TPM 密钥证明的步骤。 你还可以找到疑难解答信息和解决问题的步骤。
