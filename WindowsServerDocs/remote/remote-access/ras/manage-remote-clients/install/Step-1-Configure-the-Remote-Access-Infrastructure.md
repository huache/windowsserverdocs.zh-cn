...
使用 IPv4 和 IPv6 的环境中的单个服务器部署需要以下网络接口地址设置，具体取决于是否决定将远程访问服务器放置在边缘或 (NAT) 设备的网络地址转换后面。 可使用****“Windows 网络和共享中心”中的****“更改适配器设置”配置所有 IP 地址。
...
-   在远程访问服务器上手动配置组织 IPv4 和 IPv6 路由。 添加已发布的路由，以便将 (/48) IPv6 前缀的所有流量转发到内部网络。 此外，对于 IPv4 通信，请添加显式路由，以便将 IPv4 通信转发到内部网络。
...
-   **Teredo 流量**
    ...
    用户数据报协议 (UDP) 目标端口3544入站，以及 UDP 源端口3544出站。 为远程访问服务器上的两个面向 Internet 的连续公用 IPv4 地址应用此例外。
...
-   IPv6 (ICMPv6) 流量入站和出站的 Internet 控制消息协议仅适用于 Teredo 实现。
...
远程访问服务器和所有 DirectAccess 客户端上都需要一个证书，以便这些客户端可以使用 IPsec 身份验证。 证书必须由内部证书颁发机构颁发 (CA) 。 远程访问服务器和 DirectAccess 客户端必须信任颁发根证书和中间证书的 CA。
...
-   专用
    ...
    此证书基于你在[配置证书模板](assetId:///6a5ec5c1-d653-47b1-a567-cc485004e7bc#ConfigCertTemp)中创建的证书模板。 它需要证书吊销列表， (可以从可公开解析的 FQDN 访问的 CRL) 分发点。
...
-   证书使用者名称应该是可从外部解析的完全限定域名 (FQDN) ip-https URL (仅用于远程访问服务器 IP-HTTPS 连接的 ConnectTo 地址) 。
...
3.  在 "**新建主机**" 对话框中的 "**名称 (使用父域名（如果为空) ** ）" 框中，输入网络位置服务器网站的 DNS 名称 (这是 DirectAccess 客户端用于连接到网络位置服务器) 的名称。 在 " **IP 地址**" 框中，输入网络位置服务器的 IPv4 地址，然后单击 "**添加主机**"，然后单击 **"确定"**。

4.  在 "**新建主机**" 对话框的 "**名称 (使用父域名（如果为空) ** ）" 框中，输入 web 探测的 DNS 名称 (默认 web 探测的名称是 directaccess-webprobehost) 。 在****“IP 地址”框中，输入 Web 探测的 IPv4 地址，然后单击“添加主机”****。
...
管理员可以手动将 DirectAccess 组策略对象链接到 (OU) 的组织单位。 考虑以下情况：
...
网络位置服务器应该在具有高可用性的服务器上，并且它需要一个有效的安全套接字层 (的 SSL) 证书，该证书受 DirectAccess 客户端信任。
...
3.  设置在内部网络上 hass 高可用性的 CRL 站点。
    ...
    -   通过通用命名约定访问的文件服务器 (UNC) 路径，例如 \\ \crl.corp.contoso.com\crld\corp-APP1-CA.crl
