...
-   高可用性。 群集为访问始终\-提供高可用性。 如果群集中的服务器出现故障，远程用户可继续通过群集中的其他服务器访问企业网络。 群集中的所有服务器都具有相同的群集虚拟 IP \(VIP\) 地址，同时仍保留每个服务器的唯一专用 IP 地址。  

-   轻松\-\-管理。 群集允许将多个服务器作为单个实体进行管理。 可在各个群集服务器之间轻松共享设置。 可从群集中的任何服务器管理远程访问设置，或使用远程服务器管理工具 \(RSAT\)远程访问远程访问设置。 此外，可从单个远程访问管理控制台监视整个群集。  
...
-   对于外部负载平衡器方案，需 \(专用硬件，例如 F5 BigIP\)。  
