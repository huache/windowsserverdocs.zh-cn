...
5.  在单个网络适配器部署中，在 "**专用 IP 地址**" 页上执行以下操作，然后单击 "**下一步**"：
    ...
    1.  在 " **IPv4 地址**" 框中，输入此远程访问服务器的新 IPv4 地址;当前 IPv4 地址将是负载平衡群集 (VIP) 的虚拟 IP 地址。 在 "**子网掩码**" 框中，输入子网掩码。
...
6.  在两个网络适配器部署中，在 "**外部专用 IP 地址**" 页上执行以下操作，然后单击 "**下一步**"：
    ...
    1.  在 " **IPv4 地址**" 框中，输入此远程访问服务器的新外部 IPv4 地址;当前 IPv4 地址将是负载平衡群集 (VIP) 的虚拟 IP 地址。 在 "**子网掩码**" 框中，输入子网掩码。
...
9. 在 "**值**" 中，为网络位置服务器 (网站的 intranet 名称键入完全限定的域名 (FQDN) 例如，nls.corp.contoso.com) ，然后单击 "**添加**"。
...
禁用负载平衡将删除远程访问设置和 NLB 设置 (如果从执行它的服务器以外的所有服务器中) 配置。 在此远程访问服务器上，将删除 NLB 设置 (如果它已配置) 但远程访问设置仍将保留。

单击 "**删除配置设置**" 将删除部署中的所有服务器) 的远程访问和 NLB (。
