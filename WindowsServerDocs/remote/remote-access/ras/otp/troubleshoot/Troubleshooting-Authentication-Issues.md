...
**方案**。 用户无法使用 OTP 进行身份验证，出现错误： "由于内部错误，身份验证失败"
 ...
 (客户端事件日志) **收到错误**。 用户的 OTP 证书注册 <username> 在 CA 服务器 <上失败，CA_name>，请求失败，可能的失败原因：无法解析 ca 服务器名称，无法通过第一个 DirectAccess 隧道访问 ca 服务器，或者无法建立与 CA 服务器的连接。
...
1.  获取已配置的 OTP 颁发 Ca 的列表，并检查 "CAServer" 的值：`Get-DAOtpAuthentication`
...
**方案**。 用户无法使用 OTP 进行身份验证，出现错误： "由于内部错误，身份验证失败"
 ...
 (客户端事件日志) **收到错误**。 来自 CA <CA_name> 的证书注册失败。 请求未按 OTP 签名证书所需的方式进行签名，或者用户没有注册的权限。

**原因**

用户提供的一次性密码是正确的，但发证证书颁发机构 (CA) 拒绝颁发 OTP 登录证书。 证书请求可能未使用正确的 EKU 正确地进行签名 (OTP 注册机构应用程序策略) ，或用户没有对 DA OTP 模板的 "注册" 权限。
...
**方案**。 用户无法使用 OTP 进行身份验证，出现错误： "由于内部错误，身份验证失败"
 ...
 (客户端事件日志) **收到错误**。  无法完成 OTP 身份验证，因为在本地计算机证书存储中找不到 OTP 所需的计算机证书。
...
如果找不到有效的证书，则删除无效的证书 (如果它存在) 并通过 `gpupdate /Force` 从提升的命令提示符运行或重新启动客户端计算机重新注册计算机证书。

## <a name="missing-ca-that-issues-otp-certificates"></a>缺少颁发 OTP 证书的 CA
**方案**。 用户无法使用 OTP 进行身份验证，出现错误： "由于内部错误，身份验证失败"

 (客户端事件日志) **收到错误**。 无法完成 OTP 身份验证，因为 DA 服务器未返回发证 CA 的地址。
...
1.  使用以下命令获取颁发 OTP 证书的 Ca 的列表 (CA 名称显示在 CAServer) ：中 `Get-DAOtpAuthentication` 。

2.  如果未配置任何 Ca：

    1.  使用命令 `Set-DAOtpAuthentication` 或远程访问管理控制台来配置颁发 DIRECTACCESS OTP 登录证书的 ca。

    2.  应用新配置，并强制客户端通过 `gpupdate /Force` 在提升的命令提示符下运行或重新启动客户端计算机来刷新 DIRECTACCESS GPO 设置。
...
**方案**。 用户无法使用 OTP 进行身份验证，出现错误： "由于内部错误，身份验证失败"
 ...
 (客户端事件日志) **收到错误**。 OTP 身份验证无法按预期完成。 无法确定远程访问服务器的名称或地址。  错误代码： <error_code>。 DirectAccess 设置应由服务器管理员验证。
...
使用检查配置的 DirectAccess 服务器地址 `Get-DirectAccess` ，并更正配置错误的地址。

请确保在客户端计算机上部署最新的设置，方法是在 `gpupdate /force` 提升的命令提示符下运行或重新启动客户端计算机。

## <a name="failed-to-generate-the-otp-logon-certificate-request"></a>未能生成 OTP 登录证书请求
**方案**。 用户无法使用 OTP 进行身份验证，出现错误： "由于内部错误，身份验证失败"

 (客户端事件日志) **收到错误**。 无法初始化 OTP 身份验证的证书请求。 无法生成私钥，或用户 <username> 无法访问域控制器上的 <OTP_template_name> 的证书模板。
...
**方案**。 用户无法使用 OTP 进行身份验证，出现错误： "由于内部错误，身份验证失败"
 ...
 (客户端事件日志) **收到错误**。 无法建立与用于 OTP 身份验证的域控制器的连接。 错误代码： <error_code>。
...
-   通过在 PowerShell 提示符下运行以下命令，确保域控制器已配置为管理服务器： `Get-DAMgmtServer -Type All` 。
...
**方案**。 用户无法使用 OTP 进行身份验证，出现错误： "由于内部错误，身份验证失败"
 ...
 (客户端事件日志) **收到错误**。 使用远程访问服务器的 OTP 身份验证 ( # B0 DirectAccess_server_name 用户 (的>) ， <username>) 需要用户的质询。
...
**方案**。 用户无法使用 OTP 进行身份验证，出现错误： "由于内部错误，身份验证失败"
 ...
 (客户端事件日志) **收到错误**。 用户请求证书的 CA 模板 <username> 未配置为颁发 OTP 证书。
...
-   通过从提升的命令提示符运行以下命令来强制进行组策略更新： `gpupdate /Force` 。
...
**方案**。 用户无法使用 OTP 进行身份验证，出现错误： "由于内部错误，身份验证失败"
 ...
 (客户端事件日志) **收到错误**。 找不到 OTP 签名证书。 OTP 证书注册请求不能进行签名。
...
3.  如果此类证书不存在，请删除过期的证书 (如果存在) 并注册基于此模板的新证书。
...
-   用户 <username> 无法通过 OTP 进行身份验证。 确保为 Active Directory 中的用户名定义了 UPN。 错误代码： <error_code>。

-   用户 <username> 无法通过 OTP 进行身份验证。 确保为 Active Directory 中的用户名定义 DN。 错误代码： <error_code>。

) 收到 (服务器事件日志的**错误**

<username>为 OTP 身份验证指定的用户名不存在。

**原因**

用户不具有用户主体名称 (UPN) 或可分辨名称 (DN) 属性在用户帐户中正确设置，这些属性是正常运行 DirectAccess OTP 所必需的。
...
**方案**。 用户无法使用 OTP 进行身份验证，出现错误： "由于内部错误，身份验证失败"
 ...
 (客户端计算机) **接收到错误**。 Windows 验证你的凭据时出现问题。 请重试，或向管理员寻求帮助。
