...
ms.locfileid: "87969074"
---
# <a name="step-3-configure-the-remote-access-server-for-otp"></a>步骤3为 OTP 配置远程访问服务器

>适用于：Windows Server（半年频道）、Windows Server 2016

一旦 RADIUS 服务器配置了软件分发令牌、已创建共享机密、已在 RADIUS 服务器上创建了与 Active Directory 相对应的用户帐户，并且远程访问服务器已配置为 RADIUS 身份验证代理，则需要将远程访问服务器配置为支持 OTP。

|任务|描述|
|----|--------|
|[3.1 (可选) 豁免用户 OTP 身份验证](#BKMK_Exempt)|如果将从具有 OTP 身份验证的 DirectAccess 中免除特定用户，请遵循以下预备步骤。|
...
## <a name="31-exempt-users-from-otp-authentication-optional"></a><a name="BKMK_Exempt"></a>3.1 (可选) 豁免用户 OTP 身份验证
...
18. 在 "**开始**" 屏幕上，键入 "**powershell.exe**"，右键单击 " **Powershell**"，单击 "**高级**"，然后单击 "以**管理员身份运行**"。 如果出现了“用户帐户控制”**** 对话框，请确认其所显示的操作是你要采取的操作，然后单击“是”****。
...
若要将智能卡用于 intranet 隧道的 IPsec 隧道模式授权，必须使用智能卡基础结构 (PKI) 部署公钥基础结构。

由于 DirectAccess 客户端使用智能卡访问 intranet，因此还可以使用身份验证机制保证（Windows Server 2008 R2 的一项功能）根据用户是否使用基于智能卡的证书登录来控制对资源（如文件、文件夹和打印机）的访问。 身份验证机制保证需要 Windows Server 2008 R2 的域功能级别。
...
智能卡身份验证的工作原理是：在 DirectAccess 服务器的 intranet 隧道连接安全规则上对特定的基于 Kerberos 的安全标识符 (SID) 启用隧道模式授权。 对于智能卡身份验证，这是众所周知的 SID (S-1-5-65-1) ，它映射到基于智能卡的登录。 此 SID 存在于 DirectAccess 客户端的 Kerberos 令牌中，在 "全局 IPsec 隧道模式授权" 设置中配置为 "此组织证书"。
