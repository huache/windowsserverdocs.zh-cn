...
Windows Server 2008 R2 中引入的域控制器包括一项称为 "脱机域加入" 的功能。 使用名为 Djoin.exe 的命令行实用程序，你可以将计算机加入域，而无需在完成域加入操作的同时，通过物理方式联系到域控制器。 使用 Djoin.exe 的一般步骤如下：
...
3.  收集要应用于新客户端)  (所需的计算机证书、组策略和组策略对象。

. 以下部分介绍了使用 Djoin.exe 执行 DirectAccess 脱机加入域的操作系统要求和凭据要求。

### <a name="operating-system-requirements"></a>操作系统要求
只能在运行 Windows Server 2016、Windows Server 2012 或 Windows 8 的计算机上运行 DirectAccess Djoin.exe。 运行 Djoin.exe 将计算机帐户数据预配到 AD DS 的计算机必须运行 Windows Server 2016、Windows 10、Windows Server 2012 或 Windows 8。 要加入域的计算机还必须运行 Windows Server 2016、Windows 10、Windows Server 2012 或 Windows 8。
...
-   使用组策略授予所需的用户权限。 此方法允许你在默认的计算机容器和任何组织单位中创建计算机 (OU) ，如果不 () 添加) 添加的拒绝访问控制项，则 (。

-   编辑域的 "默认计算机" 容器 (ACL) 的访问控制列表，以将正确的权限委派给你。
...
**Domain Admins**中的成员身份或同等身份是授予用户权限的最低要求。  查看有关使用适当帐户和组成员身份的详细信息 ([本地和域默认组](https://go.microsoft.com/fwlink/?LinkId=83477) https://go.microsoft.com/fwlink/?LinkId=83477) 。
...
1.  将预配包从其保存到的远程访问服务器上 c:\files\provision.txt 复制到客户端计算机上 c:\provision\provision.txt。
