...
3. 按下表配置所需的适配器和地址。 对于使用单个网络适配器并在 NAT 设备后面设置的部署，仅使用**内部网络适配器**列配置 IP 地址。
    ...
    | IPv4 Internet 和 IPv6 Intranet | 在 IPv4 Internet 上，DirectAccess 服务器通过 Microsoft 6to4 适配器将 IPv6 路由通信转发到 6to4 中继。 可以使用以下命令为 Microsoft 6to4 适配器的 IPv4 地址配置 DirectAccess 服务器： `netsh interface ipv6 6to4 set relay name=<ipaddress> state=enabled` 。 | | |
