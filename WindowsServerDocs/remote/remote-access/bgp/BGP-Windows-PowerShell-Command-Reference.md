...
ms.locfileid: "87951412"
---
# <a name="bgp-windows-powershell-command-reference"></a>BGP Windows PowerShell 命令参考

>适用于：Windows Server（半年频道）、Windows Server 2016

在编写 Windows PowerShell 脚本时，可以使用本主题作为参考，通过 RAS 网关和远程访问局域网 (LAN) 路由器添加、配置和删除 BGP 功能。
...
为 DirectAccess (DA) 执行先决条件检查，以确保可以安装它、安装用于远程访问的 DA (RA)  (包括远程客户端管理) 或仅用于远程客户端管理，同时远程访问 VPN 和站点到站点 VPN (安装 VPN，并安装 BGP 路由。
...
> [!IMPORTANT]
> 在多租户模式下安装 RAS 网关时，必须使用**Enable-remoteaccessroutingdomain** Windows PowerShell 命令和 **-Type**参数值**All**来指定是否为每个租户启用 BGP。 下面的示例代码演示了如何在多租户模式下安装 RAS，其中的所有 RAS 功能都 (点到站点 VPN、站点到站点 VPN 和 BGP 路由) 为两个租户（Contoso 和 Fabrikam）启用。
...
卸载计算机的远程访问权限，包括 (RAS 网关、BGP 等) 的所有远程访问功能和功能。
