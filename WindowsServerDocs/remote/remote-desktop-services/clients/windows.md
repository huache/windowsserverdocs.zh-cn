...
description: 适用于 Windows 应用商店的远程桌面客户端的基本设置说明。
ms.topic: article
...
ms.locfileid: "90779251"
---
# <a name="get-started-with-the-microsoft-store-client"></a>Microsoft Store 客户端入门

>适用于：Windows 10

可以使用适用于 Windows 的远程桌面客户端以远程方式从其他 Windows 设备运行 Windows 应用和电脑。
...
3. 添加远程电脑连接或工作区。 可以使用连接直接连接到 Windows 电脑和工作区，以使用 RemoteApp 程序、基于会话的桌面或管理员发布的虚拟桌面。
4. 固定项目，以便快速访问远程桌面。

### <a name="add-a-remote-pc-connection"></a>添加远程电脑连接

创建远程电脑连接：

1. 在连接中心，点击“+ 添加”，然后点击“电脑”。 
2. 为要连接的计算机输入以下信息：
...
3. 还可以通过点击“显示更多”设置其他选项：
   ...
   - **网关** – 远程电脑网关，希望用于连接到企业内部网络中的虚拟电脑、RemoteApp 程序和基于会话的电脑。 从系统管理员那里获取有关网关的信息。
...
需要编辑这些设置？ 点击电脑名称旁边的溢出菜单 (...)，然后点击“编辑”。

想要删除连接？ 同样，点击溢出菜单 (...)，然后点击“删除”。

### <a name="add-a-workspace"></a>添加工作区

工作区包括 RemoteApp 程序、基于会话的桌面和使用远程桌面服务由管理员发布的虚拟桌面。
...
1. 在连接中心，点击“工作区”旁边的溢出菜单 ( **...** )。
2. 点击“删除”。

### <a name="pin-a-saved-pc-to-your-start-menu"></a>将保存的电脑固定到“开始”菜单

若要将连接固定到“开始”菜单，请点击电脑名称旁边的溢出菜单 (...)，然后点击“固定到开始菜单”。

现在，可以通过点击电脑连接直接从“开始”菜单启动它。
...
3. 输入以下信息：
   ...
   - **用户帐户** - 选择或添加一个用户帐户，它将用于你要连接到的远程电脑网关。 还可选择“使用电脑用户帐户”，使用远程电脑连接所用的相同凭据。
4. 点击“保存”。
...
- **显示电脑预览**：让你在连接到电脑之前，在“连接中心”看到电脑预览。 此设置默认启用。
...
当连接到电脑或工作区时，可保存帐户信息，以便稍后连接该帐户。 还可在客户端中定义用户帐户，而不是在连接到电脑时保存用户数据。
...
## <a name="navigate-your-remote-session"></a>导航远程会话

这一部分描述了可以在连接到服务后帮助你导航远程会话的工具。

### <a name="start-a-remote-session"></a>启动远程会话

1. 点击要用于启动会话的连接的名称。
2. 如果尚未保存连接凭据，系统将提示你提供用户名和密码 。
3. 如果系统要求你验证工作区或电脑的证书，请查看信息，确保你信任这台电脑，然后点击“连接”。 还可以选择“不再询问此证书”始终接受此证书。
...
### <a name="use-direct-touch-gestures-and-mouse-modes"></a>使用直接触摸手势和鼠标模式
