...
非持久性 VDI 的弊端之一是当用户注销时，几乎所有的操作系统活动都将被丢弃。 用户的配置文件和/或状态可以保存到某个中心位置，但虚拟机本身会丢弃自上次启动以来所做的几乎所有更改。 因此，针对每次切换会话都会保存状态的 Windows 计算机所做的优化并不适用。
...
若要删除 Windows Media Player 包（以释放大约 60 MB 的磁盘空间）：
...
在本地策略设置中，可以针对 VDI 中的背景禁用图像。  如果确实需要图像，可以创建色深较低的自定义背景图像，以限制用于传输图像信息的网络带宽。 如果你决定不在本地策略中指定背景图像，请在设置本地策略之前设置背景色，因为一旦设置策略，用户就无法更改背景色。 将“(null)”指定为背景图像可能更好。 下一部分介绍了不通过远程桌面协议会话使用背景的另一项策略设置。

### <a name="local-policy-settings"></a>本地策略设置

适用于 VDI 环境中 Windows 10 的很多优化都可以使用 Windows 策略来进行。 本部分的表格中列出的设置可在本地应用到基础映像/黄金映像。 如果未以任何其他方式（例如组策略）指定等效的设置，仍会应用这些设置。
...
选择以下设置不会违反安全相关的任何设置，或者与之发生冲突。 选择这些设置是为了删除或禁用不适用于 VDI 环境的设置或功能。
...
| *控制面板 | 允许在线提示 | | 已禁用。 设置不会联系 Microsoft 内容服务来检索提示和帮助内容。 |
| *控制面板\个性化 | 不显示锁屏界面 | 已启用。 此设置控制是否向用户显示锁屏界面。 如果启用此策略设置，则在登录之前无需按 CTRL + ALT + DEL 的用户在锁定电脑后会看到选定的磁贴。 |
| *控制面板\个性化 | 强制特定的默认锁屏界面和登录图像 | [![用于设置锁屏界面路径的 UI](media/lock-screen-image-settings.png)](media/lock-screen-image-settings.png) | 已启用。 使用此设置可以指定在没有任何用户登录的情况下显示的默认锁屏界面和登录图像，以及将指定的图像设置为所有用户的默认图像 - 替换默认图像。<p>建议使用低分辨率的简单图像，以减少每次呈现图像时通过网络传输的数据量。 |
| *控制面板\区域和语言选项\手写个性化 | 关闭自动学习 | | 已启用。 如果启用此策略设置，则会停止自动学习，并删除所有存储的数据。 用户无法在控制面板中配置此设置。 |
| 本地计算机策略 \\ 计算机配置 \\ 管理模板 \\ 网络 | | | |
| 后台智能传送服务 (BITS) | 不允许 BITS 客户端使用 Windows 分支缓存 |  | 启用 |
| 后台智能传送服务 (BITS) | 不允许计算机作为 BITS 对等缓存客户端 |  | 启用 |
| 后台智能传送服务 (BITS) | 不允许计算机作为 BITS 对等缓存服务器 |  | 启用 |
| 后台智能传送服务 (BITS) | 允许 BITS 对等缓存 |  | 禁用 |
| BranchCache | 启用 BranchCache |  | 禁用 |
...
| *Internet Explorer \\ Internet 控制面板 \\ 高级页 | 在网页中播放动画 |  | 禁用 |
...
| 开始菜单和任务栏 | 关闭用户跟踪 |  | 启用 |
| 开始菜单和任务栏 \\ 通知 | 关闭 toast 通知 |  | 启用 |
| Windows 组件 \\ 云内容 | 关闭所有的 Windows 聚焦功能 |  | 启用 |
...
| 策略设置 | 项 | 子项 | 可能的设置和备注|
| -------------- | ---- | -------- | ---------------------------- |
| Windows 更新或适用于企业的 Windows 更新 | 选择何时接收质量更新 | 1. 30 天<br>2.从 yyyy-mm-dd 开始暂停接收质量更新 | 启用 |
| 本地计算机策略 \\ 用户配置 \\ 管理模板 |  |  |  |
...
| Windows 服务 | 项 | 说明|
| -------------- | ---- | ---------------------------- |
| CDPUserService | 此用户服务用于互联设备平台方案 | 这是一个基于用户的服务，因此，必须禁用模板服务。  |
...
| 联系人数据 | 为联系人数据编制索引以加速联系人搜索。 如果停止或禁用此服务，搜索结果中可能会缺少联系人。 | 这是一个基于用户的服务，因此，必须禁用模板服务。  |
...
| GameDVR 和广播用户服务 | 此用户服务用于游戏录制和实时广播 | 这是一个基于用户的服务，因此，必须禁用模板服务。 |
| MessagingService | 支持短信和相关功能的服务。 | 这是一个基于用户的服务，因此，必须禁用模板服务。  |
