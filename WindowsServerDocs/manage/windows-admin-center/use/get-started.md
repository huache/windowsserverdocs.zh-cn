...
CSV 文件的格式以 ```"name","type","tags","groupId"```的四个标题开头，后跟一个新行的每个连接。
...
| 连接类型 | 连接字符串 |
|------|-------------------------------|
| WIN ENT LTSB 2016 Estonian 64 Bits | msft. 类型. 服务器 |
...
**groupId**用于共享连接。 使用此列中 ```global``` 的值，使其成为共享连接。
