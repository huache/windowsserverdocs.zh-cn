...
有关详细信息，请参阅 [Windows Admin Center](../manage/windows-admin-center/overview.md)。
...
-   有关 Windows Defender ATP 的详细信息，请参阅 [Windows Defender ATP 功能概述](/windows/security/threat-protection/windows-defender-atp/overview)。

-   若要详细了解如何载入服务器，请参阅[将服务器载入 Windows Defender ATP 服务](/windows/security/threat-protection/windows-defender-atp/configure-server-endpoints-windows-defender-advanced-threat-protection)。
...
-   [攻击面减少 (ASR)](/windows/security/threat-protection/windows-defender-exploit-guard/attack-surface-reduction-exploit-guard?ocid=cx-blog-mmpc) 是一组控件，企业可以通过它们阻止可疑的恶意文件（例如 Office 文件）、脚本、横向移动、勒索软件行为和基于电子邮件的威胁，防止恶意软件入侵计算机。

-   [网络保护](/windows/security/threat-protection/microsoft-defender-atp/network-protection)可通过 Windows Defender SmartScreen 阻止设备上的出站进程访问不受信任的主机/IP 地址，保护终结点免受基于 Web 的威胁。
...
-   [Exploit Protection](/windows/security/threat-protection/windows-defender-exploit-guard/exploit-protection-exploit-guard) 是针对漏洞利用的一组缓解措施（代替 EMET），可以轻松地进行配置以保护系统和应用程序。

[Windows Defender 应用程序控制](/windows/security/threat-protection/windows-defender-application-control/windows-defender-application-control)（也称为代码完整性 (CI) 策略）已在 Windows Server 2016 中发布。
...
有关 SDN 中新增功能的完整列表，请参阅 [Windows Server 2019 的 SDN 中的新增功能](../networking/sdn/sdn-whats-new.md)。
...
下面是存储空间直通中的新增功能的列表。 有关详细信息，请参阅[存储空间直通中的新增功能](../storage/whats-new-in-storage.md#storage-spaces-direct)。 另请参阅 [Azure Stack HCI](/azure-stack/operator/azure-stack-hci-overview)，了解如何获取经验证的存储空间直通系统。
...
- Windows Server 2019 中的[容器网络](../networking/sdn/technologies/containers/container-networking-overview.md)可增强平台网络复原和容器网络插件支持，极大地提高了 Windows 上 Kubernetes 的可用性。
...
- **使用 Windows Admin Center\(预览版\) 的管理体验**
    ...
    现在，使用 Windows Admin Center 的新扩展，用户可以比以往更轻松地查看计算机上正在运行哪些容器并管理各个容器。 查找 [Windows Admin Center 公共源](../manage/windows-admin-center/configure/using-extensions.md)中的“容器”扩展。
...
[虚拟工作负荷的网络性能提升](../networking/technologies/hpn/hpn-insider-preview.md)可最大程度地提升虚拟机的网络吞吐量，无需不断地调整或过度预配主机。 这降低了操作和维护成本，同时提高了主机的可用密度。 这些新功能包括：
...
[Windows 时间服务](../networking/windows-time-service/insider-preview.md)包括符合 UTC 标准的闰秒级支持、称为精确时间协议的新时间协议和端到端可追溯性。


### <a name="high-performance-sdn-gateways"></a>高性能 SDN 网关

Windows Server 2019 中的[高性能 SDN 网关](../networking/sdn/gateway-performance.md)极大地提高了 IPsec 和 GRE 连接的性能，在显著降低 CPU 使用率的同时提供超高性能吞吐量。
<br/>
