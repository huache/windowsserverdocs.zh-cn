...
不同之处在于，当容量管理阈值不断地超过（一次性事件不是关注点），增加容量（即在更多或更快的处理器中添加）时，将是一种解决方案，或在多个服务器之间缩放服务将是一个解决方案。 性能警报阈值表示客户端体验当前正在受到处理，需要立即采取措施来解决问题。

类似的是，容量管理是指防止汽车意外（防御驱动，确保 brakes 正常工作，等等），而性能故障排除是警察、防火部门和紧急医疗专员在发生事故后所做的工作。 这就是 "防御性驱动" Active Directory 样式。
...
简单地说，可以在 RAM 中缓存的空间越多，就越少需要再到磁盘。 若要最大程度地提高服务器的可伸缩性，最小 RAM 量应为当前数据库大小、总 SYSVOL 大小、操作系统建议量和供应商建议（防病毒、监视、备份等）之和。 应该增加额外的容量以适应服务器生存期内的增长。 基于环境更改，这将基于数据库增长的估计值，以环保为依据。
...
因此，为了最大限度地提高服务器的可伸缩性，最小 RAM 为当前数据库大小、总 SYSVOL 大小、操作系统建议数量和供应商建议（防病毒、监视、备份等）之和。 添加额外数量以适应服务器生存期内的增长。 这将根据数据库增长的估计值，以环保为依据。 但是，对于包含一小部分最终用户的卫星位置，这些要求可能非常宽松，因为这些站点不需要缓存就可为大多数请求提供服务。
...
此外，从来宾的角度来看，由于必须遍历其他代码路径，因此，必须通过主机访问任何存储，对性能产生影响。 不令人惊讶的是，存储性能测试表明虚拟化对主机系统的处理器利用率所主观的吞吐量产生了影响（请参阅附录 A： CPU 大小标准），这显然会受到来宾所要求的主机资源的影响。 这有助于在虚拟化方案中与处理需求有关的虚拟化注意事项（请参阅[用于处理的虚拟化注意事项](#virtualization-considerations-for-processing)）。
...
## <a name="processing"></a>Processing（处理）
...
此外，如果站点中的应用程序和客户端使用最佳做法来查找域控制器（即使用[DsGetDcName 函数](https://msdn.microsoft.com/library/windows/desktop/ms675983(v=vs.85).aspx)），则客户端应相对均匀地分布，因为存在任意数量的因素。
...
|网络|每秒发送的网络接口（\*） \<p>网络接口（\*） \ Received/sec|30分钟|40%|60%|
|存储|逻辑磁盘（ *\<NTDS 数据库驱动器\>* ） \Avg Disk Sec/Read<p>逻辑磁盘（ *\<NTDS 数据库驱动器\>* ） \Avg Disk Sec/Write|60分钟|10 毫秒|15 ms|
...
每个线程都是独立的任务，因为每个线程都有自己的堆栈和说明。 由于 AD DS 是多线程的，因此可以通过使用[如何在 Active Directory 中查看和设置 LDAP 策略](https://support.microsoft.com/kb/315071)来优化可用线程的数量，它可以在多个逻辑处理器之间很好地进行缩放。
...
|将块大小增加到 32 KB 后，总线成为瓶颈，因为它仅支持 20 MB/s。|添加7个磁盘（总计 = 8）<p>I/o 是随机的<p>**32 KB 块大小**<p>10000 RPM HD| |800 i/o 总数。 25600 KB/秒（25 MB/秒）可读/写到磁盘。<p>总线仅支持 20 MB/秒| | |
|升级总线并添加更多磁盘后，磁盘仍会成为瓶颈。|**添加13个磁盘（Total = 14）**<p>添加具有14个磁盘的第二个 SCSI 适配器<p>I/o 是随机的<p>4 KB 块大小<p>10000 RPM HD<p>**升级到 320 MB/s SCSI 总线**|2800 i/o<p>11200 KB/秒（10.9 MB/秒）| | | |
|将 i/o 更改为连续后，磁盘仍处于瓶颈。|添加13个磁盘（Total = 14）<p>添加具有14个磁盘的第二个 SCSI 适配器<p>**I/o 按顺序排列**<p>4 KB 块大小<p>10000 RPM HD<p>升级到 320 MB/s SCSI 总线|8400 i/o<p>33600 KB\s<p>（32.8 MB\s）| | | |
|添加更快的硬盘驱动器后，磁盘仍处于瓶颈。|添加13个磁盘（Total = 14）<p>添加具有14个磁盘的第二个 SCSI 适配器<p>I/o 按顺序排列<p>4 KB 块大小<p>**15000 RPM HD**<p>升级到 320 MB/s SCSI 总线|14000 i/o<p>56000 KB/秒<p>（54.7 MB/秒）| | | |
|将块大小增加到 32 KB 后，PCI 总线会成为瓶颈。|添加13个磁盘（Total = 14）<p>添加具有14个磁盘的第二个 SCSI 适配器<p>I/o 按顺序排列<p>**32 KB 块大小**<p>15000 RPM HD<p>升级到 320 MB/s SCSI 总线| | | |14000 i/o<p>448000 KB/秒<p>（437 MB/秒）是主轴的读/写限制。<p>PCI 总线支持理论最大值为 133 MB/秒（最高支持75%）。|
...
在为冗余设计任何系统时，还包括了附加的组件以满足故障的可能性。 在容量规划中，从可用资源中排除冗余组件非常重要。 例如，如果 SAN 有两个控制器模块，则一个控制器模块的 i/o 容量全部都应该用于系统可用的总 i/o 吞吐量。 这是因为，如果一个控制器发生故障，则所有连接的系统所需的整个 i/o 负载都需要由剩余的控制器进行处理。 由于所有容量规划都是针对高峰使用时间进行的，因此不应将冗余组件分解为可用资源，计划的高峰使用量不应超过80% 的系统饱和（为了适应突发或异常系统行为）。 同样，冗余 SAN 交换机、存储单元和心轴不应分解 i/o 计算。
