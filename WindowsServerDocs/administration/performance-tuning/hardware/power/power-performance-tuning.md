...
若要建立基线，应测量各种系统加载点（从空闲百分比到100% （最大吞吐量））所需的平均功率，以生成负载行。 下图显示了三个示例配置的负载行：
...
如果服务器需要超高延迟、固定 CPU 频率（例如，对于可重复测试）或最高性能级别，则可能不希望处理器切换到较低性能状态。 对于这种服务器，可以使用以下命令将最小处理器性能状态限制为100%：
