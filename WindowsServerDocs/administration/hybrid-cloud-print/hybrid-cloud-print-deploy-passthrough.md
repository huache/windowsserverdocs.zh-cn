...
-   作为 Active Directory 运行的 Windows Server
    ...
    有关设置 Active Directory 的帮助，请参阅[分步操作：在 Windows Server 2016 中设置 Active Directory](https://blogs.technet.microsoft.com/canitpro/2017/02/22/step-by-step-setting-up-active-directory-in-windows-server-2016/)。
...
- 步骤1：安装 Azure AD Connect 以便在 Azure AD 和本地 AD 之间同步
- 步骤2：在打印服务器上安装混合云打印包
- 步骤3：通过直通身份验证安装 Azure 应用程序 Proxy （AAP-SCHEME）
- 步骤4：配置所需的 MDM 策略
- 步骤5：发布共享打印机
...
6. 更新 `c:\inetpub\wwwroot\MopriaCloudService\web.config` 文件，使其在以下 \<运行时中包含 SQLite 版本 1.x\>/\<assemblyBinding\> 部分：
...
8. 将2个 web 应用注册到 Azure AD 以支持 OAuth2 authentication
   - 以全局管理员身份登录到 Azure AD 租户管理门户
     ...
     3. 为 native client 应用程序重复
        ...
        -   重定向 URI = "https://\<服务-计算机终结点\>/RedirectUrl"
     4. 进入本机客户端应用 "设置"
...
4. 对于 Enterprise Cloud 打印服务，请重复3个步骤，并提供企业云打印服务的内部 URL
    ...
    >   注意：以下所述的 https://&lt;services&gt;/mcs URL 应为你为 Mopria 云服务和/或企业云打印服务设置的外部 URL。
...
5. 在 Windows 10 秋季创建者更新计算机上，打开提升的 Windows PowerShell 命令提示符
   1. 运行以下命令
      ...
      - 发布-CloudPrinter
        ...
        - Sddl = 用于表示打印机权限的 SDDL 字符串。 这可以通过相应地修改 "打印机属性" "安全" 选项卡，然后在命令提示符下运行以下命令来获得： `(Get-Printer PrinterName -full).PermissionSDDL`
            例如，"G:DUD：（A; OICI; FA;;;WD） "

          > 注意：在将值设置为 "SDDL" 设置之前，需要将 **`O:BA`** 作为前缀添加到命令提示符命令的结果中。  示例： SDDL = `O:BAG:DUD:(A;OICI;FA;;;WD)`
        ...
        **发布-CloudPrinter**PowerShell 命令语法： <br>
        CloudPrinter-Printer \<string\>-Manufacturer \<string\>-Model \<string\>-OrgLocation \<string\>-Sddl \<string\>-DiscoveryEndpoint \<string\>-PrintServerEndpoint \<string\> [-AzureClientId \<string\>]\<\>\<\> <br>
...
- 打开 web 浏览器并中转到 https://&lt;服务-计算机终结点&gt;/mcs/services
- 应该会看到描述此终结点功能集的 JSON 文本
- 请参阅 "OS 设置-\> 设备-\> 打印机 & 扫描仪"
    - 应该会看到 "搜索云打印机" 链接
    - 单击链接
    - 单击 "请选择搜索位置" 链接
        - 应会看到设备位置层次结构
