...
2. **New_name\>运行重命名计算机-ComputerName \<** 。
3. 重新启动计算机。
...
|               向本地计算机添加用户                |                                                                                                                                                                                                **net user \<域 \ 用户名\> \*/add**                                                                                                                                                                                                 |
|               向本地计算机添加组               |                                                                                                                                                                                                 **net localgroup \<组名\>/add**                                                                                                                                                                                                  |
|          更改加入域的计算机的名称          |                                                                                                                                                           **netdom renamecomputer% computername% 了/newname：\<新计算机名\>/userd：\<域\\用户名\>/passwordd：** \*                                                                                                                                                            |
|                 确认新计算机名称                 |                                                                                                                                                                                                                 **set**                                                                                                                                                                                                                 |
...
|                 远程激活服务器                  |                                            **cscript slmgr.vbs-ipk \<产品密钥\>\<服务器名称\>\<用户名\>\<密码\>** <br>**cscript slmgr.vbs-ato \<servername\> \<用户名\> \<密码\>** <br>通过运行**cscript slmgr.vbs**获取计算机的 GUID-已 <br> 运行**cscript slmgr.vbs-dli \<GUID\>** <br>验证 "许可证状态" 是否设置为 "已**许可（已激活）** "。                                             |

### <a name="networking-and-firewall"></a>网络与防火墙

|任务|命令| 
|----|-------|
|将服务器配置为使用代理服务器|**netsh Winhttp set proxy \<servername\>：\<端口号\>** <br>**注意：** 服务器核心安装无法通过需要密码以允许连接的代理来访问 Internet。|
|将服务器配置为绕过 Internet 地址的代理|**netsh winhttp set proxy \<servername\>：\> 旁路列表\<端口号 "\<本地\>"**| 
...
|                         安装更新                         |                                                                                                                    **wusa \<更新\>msu**                                                                                                                    |
|                      列出安装的更新                       |                                                                                                                            **systeminfo**                                                                                                                            |
|                         删除更新                          |                                 **展开/f：\* \<更新\>msu c:\test** <br>导航到 c:\test\，在文本编辑器中打开 \<更新\>.xml。<br>将**Install**替换为**Remove** ，并保存该文件。<br>**pkgmgr.exe/n：\<更新\>.xml**                                 |
...
|                        启动任务管理器                        |                                                                                                                                                                                                           **任务管理器**                                                                                                                                                                                                            |
|    创建和管理事件跟踪会话和性能日志    | 创建计数器、跟踪、配置数据收集或 API： **logman 个** <br>查询数据收集器属性： **logman 查询** <br>开始或停止数据收集： **logman start\|stop** <br>删除收集器： **logman 删除** <br> 更新收集器的属性： **logman 更新** <br>从 XML 文件导入数据收集器集或将其导出到 XML 文件： **logman import\|export** |
...
|任务|命令| 
|----|-------|
|添加新硬件设备的驱动程序|将驱动程序复制到% homedrive%\\的文件夹中 \<driver 文件夹\>。 运行**pnputil-i-a% homedrive%\\\<驱动程序文件夹\>\\\<驱动程序\>.inf**|
|删除硬件设备的驱动程序|有关已加载的驱动程序的列表，请运行**sc query type = driver**。 然后 **\<service_name 运行 sc delete\>**|
