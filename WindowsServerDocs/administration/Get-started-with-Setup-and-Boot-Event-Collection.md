...
4.  通过转到收集器计算机并运行以下 Windows PowerShell 命令之一来测试远程连接：
    ...
    如果目标计算机与收集器计算机位于同一域中，则运行`New-PSSession -Computer <target> | Remove-PSSession`
...
5.  在目标计算机上，打开一个提升的 Windows PowerShell 提示符并运行以下命令：
    ...
    此处 <target_name> 是目标计算机的名称， \<ip> 是收集器计算机的 IP 地址。 \<port>收集器将在其中运行的端口号。 密钥 <a.b.c.d> 是通信所需的加密密钥，它包括由点分隔的四个字母数字字符串。 收集器计算机上使用此相同的密钥。 如果你未输入密钥，则系统会生成一个随机密钥；你需要将此密钥用于收集器计算机，因此请记下此密钥。
...
**现在已启用事件传输本身，你必须允许系统通过该传输实际发送 ETW 事件。**
...
-   端口 = \<port number you specified for the collector to use>
...
3.  将此示例配置复制到记事本窗口中：
    ...
    > [!NOTE]
    > 根节点为 \<collector> 。 其属性指定配置文件语法的版本和状态日志文件的名称。
    >
    > \<common>元素将多个目标组合在一起，为它们指定常见的配置元素，非常类似于用户组来指定多个用户的通用权限。
    >
...
| 错误 | 错误说明 | 症状 | 潜在问题 |
|--|--|--|--|
| Dism.exe | 87 | 在此上下文中无法识别 feature-name 选项 |  如果特性名称拼写有误，则可能会发生这种情况。 确认拼写正确，然后重试。 确认此功能在所使用的操作系统版本上可用。 在 Windows PowerShell 中，运行 `dism /online /get-features &#124; ?{$_ -match boot}`。 如果未返回匹配项，则表示正在运行的版本可能不支持此功能。 |
| Dism.exe | 0x800f080c | 功能 `<name>` 未知。 | 同上 |

### <a name="troubleshooting-the-collector"></a>收集器疑难解答

**日志记录：** 收集器将自己的事件记录为 ETW 提供程序 Microsoft-BootEvent。 你应该首先在此处尝试解决收集器问题。 你可以在事件查看器中的“应用程序和服务日志”>“Microsoft”>“Windows”>“BootEvent-Collector”>“管理员”下面查找它们，也可以在命令窗口中使用以下任一命令读取它们：
...
在调试级别，你可以将日志写入文件，而非尝试在常用的日志记录系统中查看日志。 为此，请在配置文件的元素中添加一个附加元素 \<collector> ：
...
2. 若要查看收集器的扩展诊断，请将其添加到 \<collector> 配置文件的元素中：\<collector ... minlog=verbose>
   这将启用有关每个收到的数据包的消息。
3. 检查是否确实收到了任何数据包。 （可选）你可能希望以详细模式将日志直接写入文件，而不是通过 ETW 写入文件。 为此，请将此添加到 \<collector> 配置文件的元素中：\<collector ... minlog=verbose log=c:\ProgramData\Microsoft\BootEventCollector\Logs\log.txt>
...
| 错误 | 错误说明 | 潜在问题 |
|--|--|--|
| 目标计算机 | 目标未连接到收集器 | 目标计算机在配置后不会重新启动。 重启目标计算机。 目标计算机具有错误的 BCD 设置。 检查 "验证目标计算机设置" 部分中的设置。 根据需要进行更正，然后重启目标计算机。<p>KDNET/事件-NET 驱动程序无法连接到网络适配器或连接到错误的网络适配器。 在 Windows PowerShell 中，运行 `gwmi Win32_NetworkAdapter`，并检查输出中是否有 ServiceName 为 **kdnic** 的项。 如果选择了错误的网络适配器，请重新执行中的步骤以指定网络适配器。 如果没有显示任何网络适配器，则可能是驱动程序不支持任何网络适配器。<p>**另请参阅：** 对上述收集器进行故障排除的建议方法，尤其是步骤5到步骤8。 |
...
| 收集器 | ETL 文件中未显示事件。 | 目标计算机已发送事件，但是使用消息分析器的事件查看器读取 ETL 文件时，事件不存在。 事件仍可能在缓冲区中。 在收集完整的 64 KB 缓冲区之前，或者在大约 10-15 秒的超时（超时前未发生新事件）发生之前，事件不会写入 ETL 文件。 等待超时到期，或者用 `Save-SbecInstance` 刷新缓冲区。<p>事件清单在收集器计算机或运行事件查看器或消息分析器的计算机上不可用。  在这种情况下，收集器可能无法处理事件（请查看收集器日志）或者查看器可能无法显示事件。  较好的做法是将所有清单安装在收集器计算机上，并在收集器计算机上安装更新，然后将更新安装到目标计算机上。 |
