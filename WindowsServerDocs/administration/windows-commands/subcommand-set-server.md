---
title: 子命令集-服务器
description: 子命令集的 Windows 命令主题-Server，它配置了 Windows 部署服务服务器的设置。
...
|/BindPolicy|配置 PXE 提供程序要侦听的网络接口。 使用以下选项定义策略：<p>-[/Policy： {Include &#124; Exclude}]-设置接口绑定策略，以包含或排除接口列表上的地址。<br />-[/add]-向列表添加接口。 还必须指定/addresstype 和/address。<br />-[/remove]-从列表中删除接口。 还必须指定/addresstype 和/address。<br />-/address：<IP or MAC address>-指定要添加或删除的接口的 IP 或 MAC 地址。<br />-/addresstype： {IP &#124; MAC}-指示在 **/address**选项中指定的地址类型。|
