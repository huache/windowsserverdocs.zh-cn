...
例如，若要将名为*yearly-report*的文件从 c:\reports 复制到文件共享（* \\ marketing\videos*），同时启用多线程以实现更高性能（使用/mt 参数）和重启传输以防中断（使用/z 参数），则应使用以下语法：
...
|    \<File>     | 指定要复制的一个或哪些文件。 如果需要，可以使用通配符（**&#42;** 或 **？**）。 如果未指定**文件**参数，则将** \* . \* **用作默认值。 |
