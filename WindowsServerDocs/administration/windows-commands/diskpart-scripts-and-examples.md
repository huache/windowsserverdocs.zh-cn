...
-   若要创建一个 Diskpart 脚本，请创建一个包含要运行的 Diskpart 命令的文本文件，其中每行一个命令，而不是空行。 您可以通过`rem`开始一行，使该行成为注释。  
...
启动 DiskPart 时，DiskPart 版本和计算机名称将在命令提示符中显示。 默认情况下，如果在尝试执行脚本任务时，DiskPart 遇到错误，则 DiskPart 将停止处理脚本并显示错误代码\(，除非指定了**noerr**参数\)。 但是，无论使用的是**noerr**参数，DiskPart 始终会在遇到语法错误时返回错误。 通过**noerr**参数，你可以执行一些有用的任务，例如，使用单个脚本删除所有磁盘上的所有分区，而不考虑磁盘的总数量。  
  
## <a name="additional-references"></a>其他参考
  
- [示例：使用 Windows\/PE\-和 DiskPart 配置基于 UEFI gpt 的硬盘分区](https://technet.microsoft.com/library/hh825686.aspx)  
- [示例：使用 Windows\/PE\-和 DiskPart 配置基于 BIOS MBR 的硬盘分区](https://technet.microsoft.com/library/hh825677.aspx)  
