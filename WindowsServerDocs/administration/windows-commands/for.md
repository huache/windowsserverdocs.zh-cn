...
|（\<设置>）|必需。 指定要对其运行命令的一个或多个文件、目录或文本字符串或值的范围。 需要使用括号。|
...
- 使用批处理参数
  ...
  以下属性适用于**for**命令：  
  - **For**命令会将**%**<em>变量</em>或**%%**<em>变量</em>替换为指定集中的每个文本字符串，直到指定的命令处理所有文件为止。
...
- 使用**的**其他形式
  ...
  - 仅目录
    ...
    如果*Set*包含通配符（**&#42;** 或 **？**），则会对每个目录（而不是指定目录中的一组文件）执行指定的*命令*，以匹配*集*。
  ...
  - 迭代和文件分析
    ...
    *Set*参数指定一个或多个文件名。 在移动到*集*内的下一个文件之前，将打开、读取和处理每个文件。 若要覆盖默认的分析行为，请指定*ParsingKeywords*。 这是一个带引号的字符串，其中包含一个或多个用于指定不同分析选项的关键字。
    ...
    |     usebackq      |                                                                                             指定为：执行带引号的字符串作为命令，使用带引号的字符串作为文本字符串; 或者，对于包含空格的长文件名，允许在* \<集中\>* 使用文件名，每个名称都括在双引号中。                                                                                              |
  ...
  - 变量替换
    ...
    在上述示例中，可以将 **% I**和 PATH 替换为其他有效值。 **变量名称的有效**终止**%~** 语法。

    通过使用大写变量名称（如 **% I**），可以使代码更具可读性，并避免与不区分大小写的修饰符混淆。
- 分析字符串

  可以通过以下两种方式* \<\> *使用**for/f**分析逻辑：双引号（*无*usebackq）或单引号（*带有*usebackq）--例如，（MyString）或（' MyString '） LiteralString。 LiteralString 被视为来自文件的单个行输入。 * \<\> * 在双引号中** \\ \& \| \> \< \^** 分析* \<LiteralString\> *时，命令符号（例如）被视为普通字符。
- 分析输出

  您可以使用**for/f**命令通过在括号之间放置一个反引号* \<命令\> *来分析命令的输出。 它被视为传递给子 Cmd.exe 的命令行。 将输出捕获到内存中，并将其作为文件进行分析。
...
枚举当前环境中的环境变量名称。 若要分析命令的输出，请*将集*放在括号之间，键入：
```
