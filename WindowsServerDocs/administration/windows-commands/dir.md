...
- 使用重定向符号和管道
  ...
  使用重定向符号（ **>** ）将**目录**输出发送到文件或管道（ **|** ）将**目录**输出发送到另一个命令时，请使用 **/a：-d**和 **/b**仅列出文件名。 可以使用*FileName* **和** **/s**来指定**dir**要在当前目录及其子目录中搜索与*FileName*匹配的所有文件名。 **Dir**仅列出其找到的每个文件名的驱动器号、目录名称、文件名和文件扩展名（每行一个路径）。 使用管道将**目录**输出发送到另一个命令之前，应在 autoexec.bat 文件中设置 TEMP 环境变量。
