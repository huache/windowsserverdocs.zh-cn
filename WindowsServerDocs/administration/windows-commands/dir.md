---
title: dir
description: Dir 的参考主题，其中显示了目录的文件和子目录的列表。
...
- 您可以使用通配符（**&#42;** 或 **？**）来表示文件名的一个或多个字符，并显示文件或子目录的子集。
  ...
  **星号（\*）：** 使用星号代替任意字符串，例如：  
  ...
  - **dir read\*.txt**列出当前目录中以读取和开头为 .txt （例如 .txt、. txt1 或. txt_old）开头的所有文件。
  - **目录读取\*。\* **列出当前目录中所有扩展名以 "读取" 开头的所有文件。
  ...
  你可能希望键入**dir t97\\*** 将返回文件 t97。 但是，键入**dir t97\\*** 将返回这两个文件，因为星号通配符使用其短名称映射 T97B4 ~ 1 .txt 将文件 txt2 与 t97 匹配。 同样，键入**del t97\\*** 会删除这两个文件。
...
- 指定文件名排序
  ...
  如果指定多个*SortOrder*值， **dir**会按第一个条件对文件名进行排序，然后按第二个条件对文件名进行排序，依此类推。 例如，如果将 **/o**用于值为的**e**和 **-s** *值（通过*使用 **/o： e-s**或 **/oe-s**）， **dir**会按扩展名对目录和文件的名称进行排序，首先会将其排在最前面，然后显示最终结果。 按扩展名排序的字母排序会导致不带扩展名的文件名先显示，然后是目录名称，然后是带有扩展名的文件名。
- 使用重定向符号和管道
