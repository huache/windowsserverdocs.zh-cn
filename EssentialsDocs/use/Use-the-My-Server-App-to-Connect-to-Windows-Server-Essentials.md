...
##  <a name="BKMK_WhatsNew"></a>我的服务器 2012 R2 中的新增功能  
...
2.  如果你希望能够使用服务器名称通过 intranet 连接到 Windows Server Essentials 服务器，并且你的设备具有 Windows 8 或 Windows RT 操作系统，请通过执行以下 p 在你的计算机上安装默认的 CAROOT.CER 证书rocedure. Windows 8.1 和 Windows RT 8.1 中不需要手动安装证书。  
  
###  <a name="BKMK_InstallCert"></a>在 Windows 8、Windows 8.1 或 Windows RT 设备上安装 My Server 的证书  
...
###  <a name="BKMK_Features"></a>我的服务器的功能  
...
|使用“远程桌面”连接计算机|（仅限 Server 2012 R2）打开与你的 Windows Server Essentials 服务器或客户端计算机的远程桌面会话。 （“设备” 屏幕、“远程连接” 按钮）<br /><br /> **注意：** 若要启用此功能，请在你的计算机上从适用于 Windows 的应用程序中下载并安装[远程桌面应用程序](https://apps.microsoft.com/webpdp/app/051f560e-5e9b-4dad-8b2e-fa5e0b05a480)。 标准用户帐户可以连接到它们有权登录的设备。 若要使用户能够登录某台计算机，请将该计算机添加到用户帐户的“计算机访问” 选项卡中。 有关详细信息，请参阅 [Assign user accounts permission to log on to specific network computers](../manage/Manage-Devices-in-Windows-Server-Essentials.md#BKMK_2)。|  
|管理用户|（仅限管理员）更改用户帐户的密码。 结束服务器上的用户会话。 （“用户” 设置）|  
|使用共享文件|<ul><li>从共享文件（服务器上有权访问的共享文件夹）、你的私人共享或 Windows 8.1 设备、SkyDrive 或网络存储上载和下载文件。 创建文件夹。 对服务器进行添加（上载）、编辑和删除文件。</li><li>上载或下载文件时查看传输状态。 取消传输。 解决文件冲突。</li><li>在本地计算机、服务器、SkyDrive 或网络存储上无缝使用文件和文件夹。 文件列表显示你最近在计算机上、SkyDrive 或网络存储中使用的文件夹，以及服务器上的共享文件夹，并允许你浏览其中任何位置的文件夹。</li><li>在服务器上搜索文件夹和文件；单击文件进行下载，并使用默认应用将其打开。 在脱机模式下，搜索仅限于脱机文件。</li><li>共享图片、音乐和视频。 单击文件以在 Windows 8 图片、音乐或视频播放器中将其打开。 或使用“打开” 或“打开方式” 在其他应用中打开文件。 与通常一样，你可以将选定应用设置为用于该媒体类型的默认应用。<br /><br />     **注意：** 默认情况下，媒体流功能在安装了 Windows Server Essentials Experience 角色的 Windows Server Essentials 和 Windows Server 2012 R2 中不可用。 有关详细信息，请参阅[管理数字媒体](../manage/Manage-Digital-Media-in-Windows-Server-Essentials.md)。<br /><br /> <ul><li>**图片** - 在“图片” **图片** 视图中，点击图片将其打开。 再次点击图片返回 My Server 中的缩略图视图。</li><li>**音乐**-在 "**音乐**" 视图中，查看服务器上共享的唱片集或歌曲。 点击某个项目在音乐播放器中将其打开。</li><li>**视频**-单击**视频**视图中的缩略图打开视频播放器。</li></ul></li></ul>|  
|使用 SharePoint Online 库|（仅限 Server 2012 R2）使用团队的 SharePoint Online 库中的文件。 打开团队网站。 （SharePoint Online 部分：打开团队网站，然后深化到要打开的文档库或文件。 打开文件时，必须使用与网络帐户关联的 Microsoft 联机帐户登录到 Office 365。）<br /><br /> **注意：** 若要使用此功能，服务器必须与 Office 365 集成，Office 365 订阅必须包括 SharePoint Online，并且服务器上的用户帐户必须具有与之关联的 Microsoft Online Services 帐户。 有关将 Office 365 和 SharePoint Online 与 Windows Server Essentials 集成的信息，请参阅[Windows Server essentials 的服务集成概述-第1部分](https://blogs.technet.com/b/sbs/archive/2013/11/06/services-integration-overview-for-windows-server-2012-r2-essentials-part-1.aspx)和[windows Server Essentials 的服务集成概述-第2部分](https://blogs.technet.com/b/sbs/archive/2013/11/06/services-integration-overview-for-windows-server-2012-r2-essentials-part-2.aspx)。|  
|自定义“最近” 显示内容|“最近” 列表使你可以快速访问你现在处理的文件。 你可以进行以下更改：<br /><br /> -设置要显示的最近文件历史记录的天数。 （“最近” 设置：“‘最近’中的保存天数”；默认值 = 7 天）<br />-如果不再需要处理已使用的文件，请清除 "**最近**显示"。 这不会影响缓存；这些文件仍可脱机使用。 （**最近**设置：**清除**按钮）<br />     **提示：** 如果不需要脱机访问文件，请使用 "**脱机**" 设置中的 "**全部清除**" 将文件从缓存中删除。|  
...
##  <a name="BKMK_ConnectServer"></a>如何从本地网络连接到服务器  
